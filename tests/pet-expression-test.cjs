...
test('assetFor allowlists adult-cat expression portraits only', () => {
  ...
  assert.equal(expression.assetFor('assets/characters/author/naoto.png','happy'),'assets/characters/author/naoto.png');
...
test('accentFor returns one static accessible-hidden SVG accent per non-normal adult-cat expression', () => {
  ...
  assert.equal(expression.accentFor('assets/characters/author/naoto.png','happy'),'');
});
...
test('adult dog supports ten expressions while unsupported species retain base portraits', () => {
  ...
  for (const name of ['happy','strained','sulky','hungry','sick','tired','weak','critical','wantsPlay','sleeping']) {
    ...
    assert.match(expression.accentFor(base,name),/pet-expression-accent/);
    const other='assets/characters/author/naoto.png';
...
test('sweat follows the face sides instead of the whole body', () => {
  ...
  assert.equal(expression.sweatFor('assets/characters/author/naoto.png',104,104,0),null);
});
...
for(const line of ['penguin','turtle','frog','clownfish','salmon','hermit_crab','jellyfish','starfish','coral','butterfly','beetle','stagbeetle','cicada','antlion','dandelion','sakura','venus_flytrap','mushroom','dragon','phoenix','god','world_tree','ghost','star','plush','unknown','ren'])for(let i=1;i<=8;i++)test(`${line}/${i} routes ten distinct expressions`,()=>{
...
for(let i=1;i<=8;i++)test(`unknown/${i} exposes face-relative sickness drops and the shared hunger mark`,()=>{
 const base=`assets/characters/unknown/${String(i).padStart(2,'0')}.png`;
 for(const size of [64,80,104]){
  const drops=expression.sweatFor(base,size,size,0);
