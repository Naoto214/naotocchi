...
test('real home emotion profiles select only the adult cat portrait', () => {
  ...
  const other=harness(); adultCat(other,{speciesLine:'god',ageTicks:3*20,stageIndex:1,hunger:40});
  assert.match(portrait(other),/assets\/characters\/god\/02\.png/);
...
test('form changes and blocked screens clear the temporary face and accent', () => {
  const cases = [
    ['form',h=>{Object.assign(h.api.state(),{speciesLine:'god',ageTicks:3*20,stageIndex:1});h.api.render();},'assets/characters/god/02.png'],
