...
const UNGATED = ['snow|mountain', 'forest|snow', 'forest|mountain', 'mountain|river_lake', 'desert|mountain',
  'countryside|river_lake', 'home|river_lake', 'city|countryside', 'city|sea', 'city|desert'];
// 直線が ほかの 地域の だえんを つらぬいて いる 2 本(3 章)
const PIERCING = ['forest|snow', 'countryside|river_lake'];

test('1. connection は 16 本。b あり 15 / gate 5 / 未実装 10 / special 2', () => {
  const { G } = setup();
  assert.equal(G.connections.length, 16, 'connection は 16 本');
  assert.equal(G.connections.filter((c) => c.b).length, 15, '2 地域を むすぶ ものは 15 本');
...
test('2. gate の 端点は ぜんぶで 10。出口を 1 つも もたない 地域が 6 つ ある', () => {
  const { M, G, W } = setup();
  ...
  assert.equal(ends, 10, 'gate の 端点 合計');
  assert.equal(without.sort().join(','), 'city,desert,memory_lake,mountain,river_lake,snow',
    'まだ あるいて 出られない 地域');
});

test('3. 未実装 10 本は どちらの がわにも gate が ない(片がわだけ は 0 本)', () => {
  const { M, G, W } = setup();
...
test('4. mouths の spot は ぜんぶ じっさいに あって、ひみつでは ない', () => {
  ...
  assert.equal(n, 30, 'mouth の 端点は 15 本 × 2');
});
...
test('5. anchor 候補は ぜんぶ 既存の spot で たりる(新しい spot は いらない)', () => {
  ...
  const CAND = {
    ...
    'mountain|river_lake': { mountain: ['foot', 'torii', 'mtvillage'], river_lake: ['lakelook', 'lakeshore', 'upperlook'] },
    'desert|mountain': { desert: ['gate', 'well', 'dune1'], mountain: ['windnotch', 'ridge', 'cliff'] },
    'home|river_lake': { home: ['bigtree', 'swing', 'park'], river_lake: ['riverside', 'riverbend', 'river1'] },
    ...
    'forest|snow': { forest: ['anc1', 'fernlook', 'fork'], snow: ['pines', 'snowfield2', 'lodgeback'] },
  };
  assert.equal(Object.keys(CAND).length, 10, '未実装 10 本ぶん');
...
test('6. ほかの 地域を つらぬいて いるのは forest|snow と countryside|river_lake だけ', () => {
  const { M, G } = setup();
  const R = G.regions;
  const shape = {};
  ...
  assert.equal(hits.sort().join(','), PIERCING.slice().sort().join(','),
    'この 2 本 いがいが つらぬきはじめたら 監査文書が 古い');
});
...
test('9. 通常 11 地域の 次数。一本道では なく、わが 1 つ ある', () => {
  ...
  assert.equal(N.map((id) => id + ':' + deg[id]).sort().join(' '),
    ['home:2', 'city:3', 'countryside:3', 'forest:4', 'mountain:4', 'snow:2',
      'sea:3', 'deepsea:1', 'river_lake:3', 'jungle:1', 'desert:2'].sort().join(' '));
  assert.equal(N.reduce((n, id) => n + deg[id], 0), 28, '端点 28 = link 14 本 × 2');
});

test('10. 探索率の 分母は 1 つも 動いて いない(region 11 / link 14 / tier1 17 / zone 103)', () => {
  const { M } = setup();
  const C = M.worldCountable();
  assert.equal(C.regions.length, 11);
  assert.equal(C.links.length, 14);
