...
for (const species of ['man','woman','penguin','turtle','frog','clownfish','salmon','hermit_crab','jellyfish','starfish','coral','butterfly','beetle','stagbeetle','cicada','antlion','dandelion','sakura','venus_flytrap','mushroom','dragon','phoenix','god','world_tree','ghost','star','plush','unknown','ren']) {
...
test('dandelion preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.normal.find(item=>item.id==='dandelion').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').dandelion,names);
 const html=buildPreview({preset:'hungry',form:'dandelion08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="dandelion0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'dandelion');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('sakura preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.normal.find(item=>item.id==='sakura').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').sakura,names);
 const html=buildPreview({preset:'hungry',form:'sakura08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="sakura0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'sakura');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('venus_flytrap preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.normal.find(item=>item.id==='venus_flytrap').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').venus_flytrap,names);
 const html=buildPreview({preset:'hungry',form:'venus_flytrap08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="venus_flytrap0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'venus_flytrap');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('mushroom preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.normal.find(item=>item.id==='mushroom').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').mushroom,names);
 const html=buildPreview({preset:'hungry',form:'mushroom08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="mushroom0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'mushroom');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('dragon preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.rare.find(item=>item.id==='dragon').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').dragon,names);
 const html=buildPreview({preset:'hungry',form:'dragon08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="dragon0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'dragon');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('phoenix preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.rare.find(item=>item.id==='phoenix').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').phoenix,names);
 const html=buildPreview({preset:'hungry',form:'phoenix08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="phoenix0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'phoenix');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('god preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.rare.find(item=>item.id==='god').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').god,names);
 const html=buildPreview({preset:'hungry',form:'god08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="god0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'god');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('world_tree preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.rare.find(item=>item.id==='world_tree').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').world_tree,names);
 const html=buildPreview({preset:'hungry',form:'world_tree08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="world_tree0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'world_tree');
 assert.equal(state.stageIndex,7);
 assert.equal(state.hunger,40);
 assert.deepEqual(run.calls,[]);
});

test('ghost preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.rare.find(item=>item.id==='ghost').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').ghost,names);
 const html=buildPreview({preset:'hungry',form:'ghost08'});
...
test('plush preview uses all eight canonical stage names',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.rare.find(item=>item.id==='plush').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').plush,names);
 const html=buildPreview({preset:'hungry',form:'plush08'});
 names.forEach((name,index)=>assert.match(html,new RegExp(`<option value="plush0${index+1}"[^>]*>${name}</option>`)));
 const {state,run}=seededState(html);
 ...
 test('ren preview uses canonical secret names without a fabricated rare achievement',()=>{
 const context={};
 vm.runInNewContext(fs.readFileSync(path.join(ROOT,'character-world-master.v1.js'),'utf8')+';globalThis.master=NAOTOCCHI_CHARACTER_WORLD_MASTER_V1;',context);
 const names=Array.from(context.master.playerSpecies.secret.find(item=>item.id==='ren').stages);
 assert.deepEqual(require('../tools/expression-stage-names.json').ren,names);
 const html=buildPreview({preset:'hungry',form:'ren08'});
 names.forEach((name,index)=>assert.ok(html.includes(`<option value="ren0${index+1}" ${index===7?'selected':''}>${name}</option>`)));
 const {state,run}=seededState(html);
 assert.equal(state.speciesLine,'ren');assert.equal(state.stageIndex,7);assert.equal(state.hunger,40);
 assert.equal(state.achievementsUnlocked.includes('rare-line-1'),false);
 assert.deepEqual(run.calls,[]);
});
