...
test('1. 代表ルートが 出口の いみデータを もつ(あるく 10・たてじく 2・海路 1)', () => {
  const { M } = setup();
  const withGate = G(M).connections.filter((c) => c.gate).map((c) => c.id).sort().join(',');
  assert.equal(withGate, 'city|countryside,city|desert,city|sea,countryside|forest,countryside|star_stop,deepsea|sea,desert|mountain,forest|mountain,home|forest,home|river_lake,jungle|sea,mountain|river_lake,snow|mountain');
  const walk = G(M).connections.filter((c) => c.gate && c.gate.kind === 'walk').map((c) => c.id).sort().join(',');
  assert.equal(walk, 'city|countryside,city|desert,city|sea,countryside|forest,desert|mountain,forest|mountain,home|forest,home|river_lake,mountain|river_lake,snow|mountain',
    'あるいて こえるのは 生活圏 2 + たにの 川すじ 2 + にしの 大山塊 2 + 山里・都市・海 2 + さばく 2');
