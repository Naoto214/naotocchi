...
module.exports=async function(browser,engine,fixtures,baseURL,output){
  ...
  for(const width of [320,390]){
    ...
    try{
      ...
      assert.equal(await page.locator('#itemOverlay').isVisible(),false,'shop must not cover invitation');
      ...
      legacy.oneTimeBoosts={safetyNet:true,minigameBoost:'small',greatReward:true};legacy.boostTicks=50;
      await seedNext(page,legacy,'legacy-consumables-seeded');
      await page.reload();await home(page);await openShop(page);
      assert.match(await page.locator('#itemMoneyLabel').getAttribute('aria-label'),/490/);
      await button(page,'c_life').click();saved=await readSave(page);
      assert.equal(saved.lifetime.money,490);assert.equal(stock(saved,'c_life'),2);
      ...
      await page.reload();await home(page);await openShop(page);
      assert.equal((await readSave(page)).lifetime.money,490);assert.equal(stock(await readSave(page),'c_life'),2);
