...
test('legacy sticker counts and positions merge idempotently without replacing character history',()=>{
  ...
  assert.equal(s.lifetime.pastLives[0].partner.id,'old');assert.equal(st.owned['partner:old'],1); assert.equal(h.api.stickerCatalog().length,344); assert.equal(h.api.stickerCatalog().some(x=>x.id==='item:itemluck1'),false);
});
test('sticker catalog offers all current items while retaining other collection categories',()=>{
  ...
  assert.deepEqual(catalog.reduce((counts,item)=>{
    ...
    return counts;
  },{}),{form:248,companion:26,partner:18,scenery:25});
});
