...
- 変えなかった 83 本も同じ 12 項目で点検し、判断理由を表に残した(ボード系は制限時間 60〜90 秒で操作精度を要求しない、アクション系はライフ・部分点があるなど)。
...
CT で調整した 17ID を、数字ではなく手ざわりの観点（自然な操作で成功できるか／適当操作で高得点になりすぎないか／上達が結果に出るか／理由が画面で分かるか／ガイドが答えそのものになっていないか／ガイドを見ても理不尽に失敗しないか／390px で小さすぎないか／テンポ）で見なおした。全表は `docs/qa/minigame-fun-audit-2026-09-17.md`。
...
- **ぴょんぴょんジャンプ(変更外の 83 本から発見)**。落下＝即終了で、ランダム操作では **2 秒・5 操作でゲームオーバー**していた。25 秒のゲームが 2 秒で終わると何が悪かったのか分からない。**1 回だけ助ける**ようにした(下の台に戻して「あと1回!」)。ランダム操作 10→**36**。
- ひこうき着陸の ⬛ は、点の付き方がそのまま見えるよう**的のような 3 段**(まんなか金→濃→黒)にした。

**変更しなかった 83 本の最終確認**: 上のぴょんぴょんジャンプ 1 本だけが該当。ほかはボード/パズル系は制限時間 60〜90 秒で操作精度を要求せず部分点があり、アクション系はライフか進捗点で 1 回のミスが 0 点にならず、スポーツ系は狙いを固定してから離せる。現状維持のものを無理に触るほうが手ざわりを壊すリスクが大きいと判断した。
