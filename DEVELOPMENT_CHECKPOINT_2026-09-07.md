...
- **バスケ3D**: 成功帯は払う強さ 0.35〜1.30 のうち約 5%。ゴール z=9、入る判定は半径 0.318、さらにゴールが ±1.0 揺れる。→ `HOOP_Z` 6.6 / `RIM_R` 0.55 / 判定 `-BALL_R*0.25` にし、初速を「飛ばす距離 D(3.6〜9.6)」から逆算する方式に作り直した(`T = 1.05 + D*0.03`、山なりの高さは常に一定)。払っている間に**落下点の輪**を出す。成功帯 5% → **40%**、でたらめな払いでは 11%(狙う意味は残る)。
...
- **確認**: `npm test` **839 pass**。ランダムプレイ再計測、入力レンジの数値解析、Playwright で新しいガイド(バスケの落下点リング・カーリングの停止位置●・ビーチバレーの落下点○)が実機幅 390px で描かれることをスクリーンショットで確認。
