...
現在は **全カードプールの能力本文 第1稿（Phase C）**。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、むし未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。

最新の本文は[55 ちょう／セミ／アリジゴク24枚](55-insect-three-lines-card-text-draft.md)、局面裁定・経路・持ち越しは[56](56-insect-three-lines-text-audit.md)。第1稿の手作業の点検であり、大量模擬・強度確定ではない。

**次は、[47](47-plant-four-lines-first-pass.md)を基準にタンポポ／ハエトリグサ／キノコ／世界樹の①〜⑧能力本文 第1稿。** 既存サクラと、55のちょうの運搬・アリジゴクの罠から能動型への転換を比較する。
...
既知の本文重複候補 **かめ③／さけ②、クラゲ①／サンゴ①** は未解消。正式凍結前に差別化する。55内の手札交換・一戦補助と既存カードの類似、予約の管理負担は56に残した。
...
2. げんそう未設計3種の本文。
3. ふしぎ未設計2種の本文。
4. 既存代表を含むメイン248枚の本文完備・重複・裁定の横断監査。
5. なかま／こいびと／あいてむ／あそび／セカイ／できごとの本文設計。
6. 全体の重複・裁定・強度監査、プロキシによる大量テスト。第0弾の選抜案も検証する。
7. 非アート部分を凍結。
8. 最後にカード専用イラスト。
