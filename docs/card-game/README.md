...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、63〜65の確定事項を維持し、U05の交際0開始・誘発を接続し、性別・恋愛対象を割り当てず交際条件にも使わない方針をユーザーが確定した地点**。相手ターン中の100到達の勝利期限は、2026-09-14のユーザーの「確定で」により採用済み。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
最新は[66 交際0開始・誘発と本編恋愛情報との分離](66-relationship-start-and-romance-profile.md)。01の0開始に旧05を合わせ、0→1・1→2・2→3は進行時、3→結婚は結婚時として01・02・05・06へ接続。ねこ社長・おはなの本文を08・36・37へ同期した。ユーザー確定文により**固定情報方式は不採用。各メイン・こいびとに性別・恋愛対象を割り当てず、交際成立条件にも使わない。必要な組み合わせ条件だけ個別のカード能力へ明記する。** 本編の恋愛実装は資料として保持し、カード側へ移植しない。初回21＋確定後6の計27局面を机上確認し、列挙したU05は対応済み。

**次はU07の旧A/B試験カード。** クモの準備プレイ軽減、しかける0枚のBとたぬき、トイレットペーパーの反応時点、サクラ⑥／たぬきの領域移動後の追跡、サルのコピー、とかいの使用枚数条件を点検する。0開始・誘発・性別と恋愛対象を使わない方針は再質問しない。65の途中終了、64の「100到達後に開始する最初の相手ターンの終了まで維持」、R10最終比較、63の通常行動、時0・個体別回数・期限付き効果の存続も確定済み。ほし／？？？の新規本文はこの点検後に再開する。
...
1. U07の旧試験カードを具体局面付きで改稿する。U01・U06は62、U02は63、U03は64、U04は65、U05は66で列挙事項へ対応済み。交際0開始・誘発・性別と恋愛対象を扱わない方針を再確認しない。
