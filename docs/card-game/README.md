...
現在は **再現可能な大量プロキシの入力・記録準備**。最新は[102 単独6件の先後鏡像fixture](102-single-seat-mirror-fixtures.md)。101の各P97群の`single`を、A/Bのデッキ順・初手・現物ID・初期個体IDを変えず、A後手／B先手だけへ反転した6件として固定した。101の7テストを含む12テストで鏡像不変条件と再生成一致を検査する。すべて未実施fixtureで、対戦結果や強度測定ではない。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜102は変更0 ID。次は101先手と102後手の同じP97群を1組選び、2戦の手動パイロットをcompleted形式へ記録する。

102開始時の最新main実refも `f23398e43eed952d49d5a2b805789a4c87b2b1bd` で93〜101から変更なし。取得した6 sourceの実体・blobが93の記録と一致。本編は未マージ。93時点のc77587b→f23398e差分件数は、実一覧に基づき「7」から「6ファイル」へ訂正した（94 F94-01）。
...
3. 95の未接続4 IDは96で既存供給へ接続し、97で全452件の役割重複・山札操作／予約密度・時収支・反復効率を分類した。98で採否層とschema v1、99で開始入力・参照・状態hash validator、100で物理カードIDと再登場個体IDの分離・予約の非移行、101で優先6群×3条件の18未実施fixture、102で単独6件のA後手鏡像を固定した。次は同じP97群の先手／後手1組を2戦の手動パイロットへ進める。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
4. 非アート部分を凍結。
5. 最後にカード専用イラスト。
