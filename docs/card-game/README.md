...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、全体監査後の公開情報・発動文の修正を終えた地点**。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
最新の修正は[62](62-public-information-and-activation-text-fixes.md)。61のU01・U06を、表向きのプレイ／盤面参照、探索の公開、発動した能力の解決時の実適用、対象・選択の明記で修正した。メイン26枚＋カワウソ・あたらしいであいの28候補が対象。能力の成立範囲を調整しており、強度の再試験は残る。

**次は61のU02、通常行動・支払い・反応の手順を、カブト④等の具体局面で整理する。** U03開始／終了、U04ちょうせん中の入替、U05交際・恋愛タイプ、U07旧試験カードも残る。ほし／？？？の新規本文はその点検のため中断中。大きな共通ルール判断だけ確認し、既決定の時0・個体別回数・期限付き効果の存続は再質問しない。
...
特にカブトは[15 第2稿](15-test-deck-b-main-16-draft.md)、クワガタは[23 第3稿](23-stagbeetle-first-pass.md)が現行の移植元。62のカブト⑧の対象語彙は15・31・08へ同期済み。[24](24-beetle-vs-stagbeetle-simulation-1.md)はクワガタ第2稿から問題を検出した記録で、第3稿より新しいカード本文ではない。31はこの2稿をM-IDへ接続した管理文書で、能力を再設計したものではない。
...
1. 61の残件U02〜U05・U07を、具体局面付きで整理・改稿する。U01・U06の本文反映は62で実施済み。
