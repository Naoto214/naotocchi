...
現在は **再現可能な大量プロキシの小規模実施**。最新は[109 同名2枚6群の固定意思決定パイロット](109-same-name-two-pilots.md)。101のA先手`same-name-two` 6 fixtureから前提カード位置だけを交換し、全群で別現物2枚の解決を判断・状態traceへ接続した。P97-02は予約2本を消費、P97-06は同一対象への2本を1本消費・1本失効、P97-05は再登場2回。固定線6戦は独立した勝率標本ではなく、勝率・先後差・発動率・カード強度の結論には数えない。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜109は変更0 ID。`single` 6群は103〜106で先後各1戦、計12統制記録へ接続した。108の通常意思決定2戦、109の同名2枚6戦も固定線completed記録で、独立した強度標本には数えない。次はP97-01〜06の盤面併用6群へ広げる。
