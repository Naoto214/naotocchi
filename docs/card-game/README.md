...
現在は **全カードプールの能力本文 第1稿（Phase C）で、メイン248枚の本文案と横断監査第1周を保存し、なかま26候補の全件本文へ進む地点**。01〜24の基本ルール・8属性代表・A/B模擬・カブト／クワガタを基準に、未展開だった58枚を68〜70で具体化した。63〜66のユーザー確定事項、67の旧A/B修正は維持する。非アート部分と強度は未凍結。カード専用の最終イラストは最後に行う。

最新は[71 メイン248枚の本文横断監査](71-main-248-card-text-cross-audit.md)。カブト②・ヤドカリ②④・いぬ⑥・かえる⑥の5本文を整理し、れんくん8枚はカード側通常・各同名3枚の第1稿として明記。追加32局面を机上照合した。山札操作密度・低段階の高い値・？？？⑦の二方向の強さ・準備やつけ替えの供給は実測課題として残す。
...
**次は35のなかま26候補の全件本文、その次に36のこいびと18候補。** 70でAの3省略本文と本編形態対応を整理し、71でメイン横断監査を行った。既存A/Bの本文を土台に、装備先・準備・対象変更・サルの有効なコピー先の供給不足も追跡する。U05の0開始・誘発・性別と恋愛対象を使わない方針、65の途中終了、64の100維持期限・R10比較、63の通常行動は再質問しない。
...
1. なかま26の全件本文、続いてこいびと18の全件本文。既存A/Bの現行本文を土台にし、71の供給・コピー・準備の課題へ接続する。
