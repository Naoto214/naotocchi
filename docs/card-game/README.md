...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、全体監査後の修正、通常行動・反応の確定、開始／終了・予約・期限・勝利判定を基本ルールへ反映した地点**。相手ターン中の100到達の勝利期限も、2026-09-14のユーザーの「確定で」により採用済み。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
最新は[64 開始／終了・予約・勝利判定](64-turn-boundaries-and-victory-timing.md)。初期01の開始順を復元し、自分の時回復→通常ドロー→たまご交換→予約→誘発・反応、終了前の反応→終了予約・誘発・反応→期限切れ→判定へ接続した。01・02・06へ反映し、23の③は本文を維持して残り時の確認時点を注記。28局面を手作業で照合した。

**次はU04ちょうせん中の入替・無指定のちょうせん表記。** 64第5節は「100到達後に開始する最初の相手ターンの終了まで維持」で確定し、01・06へ反映済み。到達時に進行中の相手ターンは含めず、判定がR10なら最終比較へ進む。自分ターン中の100到達・R10判定、63の通常行動方式は再提案しない。U05交際・恋愛タイプ、U07旧試験カードも残る。ほし／？？？の新規本文は点検のため中断中。既決定の時0・個体別回数・期限付き効果の存続は再質問しない。
...
おとこのひと・カブト・クワガタと、いぬ／かえる／カクレクマノミ／サクラ／フェニックス／ぬいぐるみの既存代表稿・A/B採用本文は比較基準として維持する。本文案ありは**190枚**（新規160＋既存3種24＋その他代表のA採用6）。この6枚には省略本文の正式化も残る。**未展開58枚**は、ほし／？？？16枚と、その他6代表の残り42枚。248枚すべての本文が完成した状態ではない。
...
1. 61 U04の参加メイン離脱・無指定ちょうせん表記へ進む。U05・U07も具体局面付きで整理・改稿する。U01・U06は62、U02の個別修正と通常行動の確定は63、U03の開始／終了・予約・期限と勝利期限の確定は64で反映済み。
