...
現在は **再現可能な大量プロキシの小規模実施**。最新は[108 通常意思決定を含む先後2戦](108-normal-decision-match-pair.md)。107の手順と既存A/B各40枚・先後鏡像を維持し、通常行動機会ごとに合法候補と`pass`、判断前状態、選択、次点、理由、支払い、対象、連鎖、公開情報を記録した。A先手は45対35、A後手は40対35で、判断不能0・予約0・再登場0。固定線2戦は独立した勝率標本ではなく、勝率・先後差・発動率・カード強度の結論には数えない。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜108は変更0 ID。`single` 6群は103〜106で先後各1戦、計12統制記録へ接続した。108の通常意思決定2戦は別の固定線completed記録で、独立した強度標本には数えない。次はP97-01〜06の同名2枚6群、その後に盤面併用6群へ広げる。
