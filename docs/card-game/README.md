...
現在は **再現可能な大量プロキシの入力・記録準備**。最新は[107 通常意思決定を含む小規模対戦プロトコル](107-normal-decision-match-protocol.md)。各通常行動機会で合法候補と「何もしない」を列挙し、判断前状態、選択、次点、理由、支払い、対象、連鎖、公開情報を記録する手順を固定した。判断優先順位、理由語彙、停止条件、評価項目と、既存A/B各40枚・先後鏡像を維持する未実施fixture 2件を8テストで検査する。107ではcompleted対戦を作らず、勝率・先後差・発動率・カード強度の結論には数えない。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜107は変更0 ID。`single` 6群は103〜106で先後各1戦、計12統制記録へ接続した。次は108で通常意思決定を含む先後2戦をcompleted記録にし、経路が安定してから同名2枚、盤面併用へ広げる。
