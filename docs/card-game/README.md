...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、63〜65の確定事項を維持し、U05のユーザー確定方針を反映し、U07の旧試験カード6本文と次回試験用Bの1枠改訂まで進んだ地点**。相手ターン中の100到達の勝利期限は、2026-09-14のユーザーの「確定で」により採用済み。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
最新は[67 旧A/B試験カードの機能・追跡・コピー・使用枚数](67-legacy-test-card-function-and-tracking.md)。クモの次の準備プレイ軽減、トイレットペーパーの除去発動への反応、サクラ⑥／たぬきの発動に伴う領域移動の追跡、サルの能力取得と別発動、とかい固有の2枚集計を明文化した。次回試験用Bはたぬき1枚を2枚目のふくろうへ差し替え、旧B40枚と対戦ログは保存。列挙したU07へ対応し、46局面を机上照合した。全カードの裁定完備・強度確定やA/Bの再対戦ではない。

**次はほし／？？？16枚、既存6代表の残り42枚を展開し、メイン248枚の本文横断監査へ進む。** 他の省略本文・準備札の低発動率・コピーの有効な組み合わせ・強度もそこで点検する。U05の0開始・誘発・性別と恋愛対象を使わない方針、65の途中終了、64の100到達後に始まる最初の相手ターン終了までの維持とR10最終比較、63の通常行動、時0・個体別回数・適用済み効果の存続は再質問しない。
...
おとこのひと・カブト・クワガタと、いぬ／かえる／カクレクマノミ／サクラ／フェニックス／ぬいぐるみの既存代表稿・A/B採用本文は比較基準として維持する。本文案ありは**190枚**（新規160＋既存3種24＋その他代表のA採用6）。A採用6のうち、65でフェニックス⑤・ぬいぐるみ⑦、67でサクラ⑥を省略のない本文へ進めた。いぬ②・かえる③・カクレクマノミ④の正式化と、全体の関連裁定は残る。**未展開58枚**は、ほし／？？？16枚と、その他6代表の残り42枚。248枚すべての本文が完成した状態ではない。
...
1. ほし／？？？16枚と既存6代表の残り42枚を展開する。
2. メイン248枚の本文完備・重複・裁定を横断監査する。既存A採用の省略本文とコピー・反応・効果追跡もここで再点検する。
3. なかま／こいびと／あいてむ／あそび／セカイ／できごとの本文設計。
4. 全体の重複・裁定・強度監査、プロキシによる大量テスト。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
5. 非アート部分を凍結。
6. 最後にカード専用イラスト。
