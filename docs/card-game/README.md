...
現在は **再現可能な大量プロキシの入力・記録準備**。最新は[106 P97-01／02／04単独パイロット](106-remaining-single-pilots.md)。101先手と102後手に残っていた3群を先後各1戦、計6 completed記録へ固定した。P97-01は交際0のこいびと配置→はじめてのデート、P97-02はミニゴルフの同ターン終了予約、P97-04はメイン・とかい・表向き装備2枚→とかいランの異種2枚取得を記録する。42／46 event、43／47 snapshot、状態hashの連続と12 JSONの再生成一致を7テストで検査する。統制記録であり、勝率・先後差・発動率・カード強度の結論には数えない。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜106は変更0 ID。`single` 6群は103〜106で先後各1戦、計12統制記録へ接続した。次は通常の意思決定を含む小規模対戦を定義し、その後に同名2枚、盤面併用へ広げる。
