...
現在は **[127 R2候補拡張と独立再開](127-r2-candidate-extension.md)** の保存地点。盤面枚数条件の除外理由と単一対象IDを追加証明し、01-Bの5候補を107/114/116で比較した。126の4停止点から独立再開し、01-Aは次の相手ターンへ、01-Bは無料配置と応答へ進んだ。02-Aは対象なしセカイ配置ID、02-Bは種類の異なる候補の比較証明、01の2経路は能力なしなかまの盤面分類で真正停止した。completed・独立balance標本は0。
