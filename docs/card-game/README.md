...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、ルール・本文・取り決めの全体監査を実施した地点**。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
**次は61のU01〜U07を基準に、公開情報・行動手順・開始／終了・ちょうせん中の入替・交際・発動本文・A/B試験カードの不足を整理する。** ほし／？？？の新規本文は、その点検のためいったん中断した。大きな共通ルール判断だけ確認し、既決定の時0・個体別回数・期限付き効果の存続は再質問しない。
...
1. 61で見つかった共通フローと既存本文の不足を、具体局面付きで整理・改稿する。
