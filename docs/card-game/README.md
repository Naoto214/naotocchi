...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、全体監査後の修正、通常行動・反応の確定、開始／終了・予約・期限の手順を基本ルールへ反映した地点**。相手ターン中の100到達の勝利期限だけは確認待ち。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
**確認待ちは「相手ターン中に100へ届いた場合、到達後に開始する相手ターンの終了まで維持を求めるか」の一点。** 64第5節に推奨案と勝敗が変わる例を保存した。自分ターン中の100到達・R10判定、63の通常行動方式は再提案しない。確認後はU04ちょうせん中の入替・無指定のちょうせん表記へ進む。U05交際・恋愛タイプ、U07旧試験カードも残る。ほし／？？？の新規本文は点検のため中断中。既決定の時0・個体別回数・期限付き効果の存続は再質問しない。
...
1. 64第5節の相手ターン中100到達の勝利期限だけを確認・反映し、61 U04の参加メイン離脱・無指定ちょうせん表記へ進む。U05・U07も具体局面付きで整理・改稿する。U01・U06は62、U02の個別修正と通常行動の確定は63、U03の開始／終了・予約・期限の共通手順は64で反映済み。
