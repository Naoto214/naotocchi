...
現在は **全種類横断監査で、除去・防御の実在供給を照合した地点**。最新は[94 除去・防御の実在供給監査](94-removal-defense-supply-audit.md)。相手メイン・なかまの除去、こいびと／セカイの捨て札行きは現行本文の供給がなく、主要機能14件・部分機能2件・対象変更の曖昧さ1件の計17 IDを要追従として整理した。準備除去・関係の手札戻し・五目のそだち減少には実在する防御経路がある。40局面は机上確認で、発動率・大量対戦・強度確定ではない。次は94 F94-02〜05の個別本文調整と、役割・山札操作／予約密度・時収支の監査。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。94ではカード本文・数値・登録区分を変更していない。ボウリングの印刷時1以下の装備は現行14装備にはなく、旧おはな等を明示採用した試験と分ける。

94開始時の最新main実refは `f23398e43eed952d49d5a2b805789a4c87b2b1bd` で93保存時から変更なし。取得した6 sourceの実体・blobが93の記録と一致。本編は未マージ。93時点のc77587b→f23398e差分件数は、実一覧に基づき「7」から「6ファイル」へ訂正した（94 F94-01）。
...
3. 94で特定した供給不足・ボウリングの適正対象・対象変更・ゆきだるまの個別本文を調整し、全種類の重複・裁定・強度監査、再現可能なプロキシによる大量テストへ進む。メイン横断監査第1周で残した密度・強度も測る。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
4. 非アート部分を凍結。
5. 最後にカード専用イラスト。
