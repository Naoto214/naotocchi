...
現在は **再現可能な大量プロキシの通常意思決定fallback contract完了地点**。最新は[116 通常意思決定fallback contract](116-normal-decision-fallback-contract.md)。115で停止したR1たまご交換の複数選択を`mandatory_choice`として記録し、合法候補を完全列挙できる比較不能は判断単位のseed抽選で処理するprotocolを固定した。安全な時0人物配置は6安全条件を満たす場合だけ`pass`より上位とし、複数の安全候補はseed抽選へ送る。116のfixture・completed・trace・event・snapshot・winner・独立balance標本は0件で、112の対象fixture 6件はevent空・winner nullの未実施、115の4停止経路は未再開を維持する。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜115は変更0 ID。`single` 6群は103〜106で先後各1戦、計12統制記録へ接続した。108の通常意思決定2戦、109の同名2枚6戦、110の盤面併用6戦は固定線completed記録で、独立した強度標本には数えない。113は入場監査、114はprotocol hardening、115はfirst-choice監査だけでcompleted 0件。101の3条件×6群は固定completed記録へ接続済みだが、全24戦を通常の独立対戦へ数えない。
