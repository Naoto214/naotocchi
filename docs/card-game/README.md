...
現在は **全カードプールの能力本文 第1稿（Phase C）**。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、くさ未設計4種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。

最新の本文は[57 タンポポ／ハエトリグサ／キノコ／世界樹32枚](57-plant-four-lines-card-text-draft.md)、局面裁定・経路・持ち越しは[58](58-plant-four-lines-text-audit.md)。第1稿の手作業の点検であり、大量模擬・強度確定ではない。

**次は、[48](48-fantasy-three-lines-first-pass.md)を基準にりゅう／かみさま／おばけの①〜⑧能力本文 第1稿。** 既存フェニックスを維持し、57の世界樹の高基礎値に対する条件・代償、キノコの領域横断・離脱後の予約と比較する。
...
| 棚卸し区分 | 候補数 | 現在の扱い |
|---|---:|---|
| メイン | 248 | 31種×8段階。55の24枚・57の32枚も内数 |
| なかま | 26 | 役割地図まで |
| こいびと | 18 | 役割地図まで |
...
既知の本文重複候補 **かめ③／さけ②、クラゲ①／サンゴ①** は未解消。正式凍結前に差別化する。55内の手札交換・一戦補助と既存カードの類似、予約の管理負担は56に、57の循環・反復への無効化・多種類条件・紙での情報負担は58に残した。
...
1. げんそう未設計3種の本文。
2. ふしぎ未設計2種の本文。
3. 既存代表を含むメイン248枚の本文完備・重複・裁定の横断監査。
4. なかま／こいびと／あいてむ／あそび／セカイ／できごとの本文設計。
5. 全体の重複・裁定・強度監査、プロキシによる大量テスト。第0弾の選抜案も検証する。
6. 非アート部分を凍結。
7. 最後にカード専用イラスト。
