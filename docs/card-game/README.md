...
現在は **全カードプールの能力本文 第1稿（Phase C）で、あそび100候補の第1組20枚を本文化し、40局面の接続監査を保存した地点**。あそびは新20＋既存2048で本文あり21/100、未展開79。メイン248・なかま26・こいびと18・現行アイテム26の保存済み本文を維持する。アイテム26は登録14＋未登録source12、旧退役source24は履歴を保持。登録済み477へ新規sourceを自動加算しない。全裁定・強度は未確定、専用イラストは最後。

最新は[79 あそび第1組20本文](79-play-batch-1-card-text-draft.md)・[80 接続監査40局面](80-play-batch-1-text-audit.md)。最新main `5d39eea915d09be3900d92fa879a9e9f1ecf6091`（#285）の100 ID・category・source区分と対象20の操作・判定を照合した。ケーキ／おべんとうの役割説明を実装に合わせ、積み上げ・探索・準備干渉・限定反応へ分けた。時1が7枚・時2が13枚、すぐつかう18・しかける2、通常20の第1稿。次は39の21番G-basketball-3dから。旧A/Bの5あそびは維持し、legacy4を本文21へ加算しない。
...
2. あそびは79・80の次、39の21番G-basketball-3dから未展開79枚を続ける。46番2048は既存本文を再利用し二重加算しない。その後セカイ／できごとの全件本文。73 V01・75 V02等の共通未記載事項も大量プロキシ前に解消する。
