...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、全体監査後の修正と、通常行動の共通手順案を保存した地点**。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
最新は[63](63-action-payment-response-proposal.md)。カブト④を支払い時の任意軽減と移動終了後の禁止へ改稿し、15・31・08を同期。さけ③⑦の通常セカイ配置、ヒトデ⑤のアクション軽減を明記し、シルクハットの支払い時点を注記した。個別処理19局面と、共通案を採用した場合の8局面を整理した。

**次は63第4節の共通案を確認する。** 推奨案は「通常の人生移動・配置等は自分の行動機会に実行し、反応はすぐつかう・条件を満たす能力・しかけた札の発動で行う」。相手ターンや連鎖中の通常へんしんを許すかは勝負全体に影響するため、01・02・06・07へ未反映。承認後に共通手順へ接続し、U03開始／終了へ進む。U04ちょうせん中の入替・無指定のちょうせん表記、U05交際・恋愛タイプ、U07旧試験カードも残る。ほし／？？？の新規本文は点検のため中断中。既決定の時0・個体別回数・期限付き効果の存続は再質問しない。
...
| [63](63-action-payment-response-proposal.md) | U02のメイン4本文を改稿し、帽子の支払いを注記。19局面と共通案8局面。通常行動の機会・反応手順案は確認待ち |
...
1. 63第4節の通常行動・反応の共通案を確認し、承認後に基本ルールへ接続する。続いて61の残件U03〜U05・U07と、無指定ちょうせん表記を具体局面付きで整理・改稿する。U01・U06の本文反映は62、U02の個別コスト／語彙修正は63で実施済み。
