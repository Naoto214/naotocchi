...
現在は **全カードプールの能力本文 第1稿（Phase C）で、メイン248枚・なかま26候補・こいびと18候補の全件本文と接続監査第1周を保存し、あいてむの全件本文へ進む地点**。01〜24の基本ルール・8属性代表・A/B模擬・カブト／クワガタを基準に、未展開だったメイン58枚を68〜70、なかま20体を72、こいびと14体を74で具体化した。63〜66のユーザー確定事項、67の旧A/B修正は維持する。非アート部分と強度は未凍結。カード専用の最終イラストは最後に行う。

最新は[74 こいびと18候補の本文](74-partner-18-card-text-draft.md)と[75 接続監査・40局面](75-partner-18-text-audit.md)。ねこ社長・クモの全文は保持し、にんぎょ・ゆきだるまの省略文を具体化して36・08へ同期。残り14体も全文へ進み、能力本文18、能力なし0。カード側構築区分は全18体とも通常・同名3枚の第1稿として27へ明記した。タコの適正なつけ替え、サボテンとすれちがい等を接続し、交際・結婚・たまご・個体追跡・時の収支を照合。強度・除去やサルの装備供給は未解決で、75 V02にはたまご化と未解決こいびと能力の無効範囲を記録した。73 V01とともに大量プロキシ前に明文化する。
...
**次は本編アイテム変更の実状を再確認し、28・37のあいてむ38候補と既存A/B本文から全件本文へ進む。** 変更連絡時の実差分へ追従し、未提示の改定内容は推測しない。装備先・準備・対象変更・サルの有効なコピー先の供給不足、クマ等の除去・ゆきだるまの関係終了の起点、73 V01・75 V02を追跡する。U05の0開始・誘発・性別と恋愛対象を使わない方針、65の途中終了、64の100維持期限・R10比較、63の通常行動は再質問しない。
...
あいてむ等の「役割地図まで」は全体の進捗で、A/Bで既に具体本文を試しているカードも含む。旧テストを未設計に戻す意味ではない。
...
1. 本編アイテム変更の実状を確認し、あいてむ38候補と既存A/B本文から全件本文へ。71〜75の供給・コピー・準備の課題へ接続する。変更は実際の連絡・差分を確認して反映する。
2. あそび／セカイ／できごとの全件本文。73 V01・75 V02等の共通未記載事項も大量プロキシ前に解消する。
