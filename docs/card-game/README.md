...
現在は **全種類横断監査の第1段階で、3件の処理境界を具体化した地点**。最新は[93 横断・境界監査](93-cross-type-boundary-audit.md)。なかま満員交代の退出先を01へ、たまごと未解決こいびと能力の確認時点を02・06へ明文化し、2048の交換を効果として08・83へ全文化した。32局面は机上確認であり、大量対戦ではない。次は除去・関係終了等の実在する供給と必要な防御、役割重複・時収支の監査。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。現行アイテムの未登録source12を含み、旧退役24・HOLD・legacyは別管理。全裁定・強度・非アート凍結は未確定。保存直前の最新main `f23398e43eed952d49d5a2b805789a4c87b2b1bd`（#288）はめぐるの見切れ修正。7ファイルの差分とsource blobを確認し、カードに関わる定義・効果の追加変更なし。根拠は93へ保存。本編は未マージ。
