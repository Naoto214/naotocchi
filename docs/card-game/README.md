...
現在は **再現可能な大量プロキシの小規模実施**。最新は[112 構造的入力不足6件の対象fixture](112-targeted-structural-gap-fixtures.md)。111の短期対象2件・適正な段階経路4件を、現行452内の未実施fixture各1件へ固定した。A/B各40枚・7種類、card copy／initial instance IDを保持し、6件ともevent空・winner nullである。completed対戦・発動率0・カード強度には数えない。次は111どおり、独立初期順2組×先後鏡像＝4戦を通常意思決定の最初の1バッチとする。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜112は変更0 ID。`single` 6群は103〜106で先後各1戦、計12統制記録へ接続した。108の通常意思決定2戦、109の同名2枚6戦、110の盤面併用6戦も固定線completed記録で、独立した強度標本には数えない。101の3条件×6群は固定completed記録へ接続済みだが、全24戦を通常の独立対戦へ数えない。
