...
現在は **再現可能な大量プロキシの入力・記録準備**。最新は[99 プロキシ記録validator第1段階](99-proxy-record-validator.md)。98のschema v1を読む標準ライブラリだけのvalidatorを追加し、版、各40枚、先後、初手、現行452 ID、7種類、80個体、選択、event連番、連鎖、予約、状態hash、結果の参照整合を検査する。カード効果は実行せず、再登場で生じる新個体の生成履歴はv1未対応として明記した。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜99は変更0 ID。次は物理カードの不変IDと、場へ出るたび更新する個体IDを分けてから、97の優先6群を再現fixtureへ落とす。

99開始時の最新main実refも `f23398e43eed952d49d5a2b805789a4c87b2b1bd` で93〜98から変更なし。取得した6 sourceの実体・blobが93の記録と一致。本編は未マージ。93時点のc77587b→f23398e差分件数は、実一覧に基づき「7」から「6ファイル」へ訂正した（94 F94-01）。
...
3. 95の未接続4 IDは96で既存供給へ接続し、97で全452件の役割重複・山札操作／予約密度・時収支・反復効率を分類した。98で採否層とschema v1、99で開始入力・参照・状態hash validatorを固定した。次は物理カードIDと再登場個体IDを分け、旧個体だけを追う予約を検査してから97の優先6群を再現fixtureへ落とす。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
4. 非アート部分を凍結。
5. 最後にカード専用イラスト。
