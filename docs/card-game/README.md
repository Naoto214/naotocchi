...
現在は **全種類横断監査の供給不足を個別改稿している地点**。最新は[96 除去・防御の期限と行先の第2次改稿](96-removal-defense-deadline-revisions.md)。95で実適用経路が未接続だったペンギン⑦・タンポポ④・とけないゆきだるま・ハイウェイドライブの4 IDを、既存のりゅう⑥・とつぜんのたび・すれちがいへ接続した。28局面は机上確認。新しい除去札や自由な通常別れは追加していない。95の未接続4 IDは0になったが、メイン手札戻し、直接のなかま手札／山札移動、こいびと／セカイ捨て札行きの供給は引き続き0で、全条件の供給完備や強度確定ではない。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 IDに続き、96は既存4 IDだけを改稿し、メイン数値・通常時・登録区分を維持した。ボウリングの対象となる現行装備は印刷時2の8種。94の旧版では適正装備0だった履歴と、旧おはな等のlegacy試験を区別する。

96開始時の最新main実refも `f23398e43eed952d49d5a2b805789a4c87b2b1bd` で93〜95から変更なし。取得した6 sourceの実体・blobが93の記録と一致。本編は未マージ。93時点のc77587b→f23398e差分件数は、実一覧に基づき「7」から「6ファイル」へ訂正した（94 F94-01）。
...
3. 95の未接続4 IDは96で既存供給へ接続した。残る部分供給、全種類の役割重複・山札操作／予約密度・時収支・反復効率を整理し、新12・旧24・HOLD14・legacy4の区分を保って採否を整理する。その後、再現可能なプロキシによる大量テストへ進む。メイン横断監査第1周で残した密度・強度も測る。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
4. 非アート部分を凍結。
5. 最後にカード専用イラスト。
