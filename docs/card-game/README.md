...
現在は **全カードプールの能力本文 第1稿（Phase C）の途中で、全体監査後の修正と通常行動・ターン境界の確定を経て、ちょうせんの本文4枚を改稿し、宣言時の開始・比較前の離脱による途中終了・終了と完了の区別を確定して基本ルールへ反映した地点**。相手ターン中の100到達の勝利期限は、2026-09-14のユーザーの「確定で」により採用済み。基本ルール、8属性代表テスト、A/B模擬、カブト／クワガタ近縁差別化を基準に全体へ広げている。31種の人生テーマ・仮数値の第1周は完了し、げんそう未設計3種の具体本文まで進んだ。非アート部分は未凍結。カード専用の最終イラストは最後に行う。
...
最新は[65 ちょうせんの参加個体・本文表記と終了](65-challenge-participants-and-resolution.md)。おとこのひと⑥・ヒトデ⑥・フェニックス⑤の自分から／受ける時を明記し、ぬいぐるみ⑦とともに4カードを改稿。15・54・08へ反映し、02へ表記を接続した。共通裁定は2026-09-14のユーザー提示文「これで」により確定し、01・02・06へ反映済み。**勝負は宣言時に開始し、比較前の参加メイン離脱で途中終了する。宣言回数・支払済みコストは戻さず、適用済みの終了時・終了後効果は条件どおり処理する。比較・勝敗の成立が必要な効果は、途中終了では働かない。** 初回36＋追加6の42局面を机上確認し、「終了」と比較・結果決定を経た「完了」を区別した。

**次はU05交際0開始・0→1の誘発と恋愛タイプ、U07旧試験カードへ進む。65の途中終了は確定・反映済みで再質問しない。** 64第5節の「100到達後に開始する最初の相手ターンの終了まで維持」は確定・01・06反映済み。到達時に進行中の相手ターンは含めず、判定がR10なら最終比較へ進む。勝利期限・R10判定、63の通常行動方式、時0・個体別回数・期限付き効果の存続は再質問しない。ほし／？？？の新規本文は点検のため中断中。
...
1. U05・U07を具体局面付きで整理・改稿する。U01・U06は62、U02は63、U03は64、U04の個別4本文と途中終了・結果・予約・終了の共通裁定は65で反映済み。
