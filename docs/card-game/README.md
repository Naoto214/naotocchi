...
現在は **通常意思決定seeded restartの4経路停止・評価保存地点**。最新は[117 通常意思決定seeded restart](117-normal-decision-seeded-restart.md)。115と同じsource・seed・40枚manifestへ116を適用し、R1 mandatory choiceと安全配置を4経路とも通過した。実結果はplanned 4・completed 0・stopped 4・独立balance標本0。全経路が配置直後のpost_placement_responseで候補／phase／stable ID contract不足により停止し、winnerなしのstop 4件とevaluationを保存した。completed record／traceは作らず、停止までの観測値を勝敗数や発動0の完走対戦に数えない。112の6 fixtureは未実施のまま。専用24件・全proxy 190件PASS、CLI valid・総合checker errors=[]。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97〜117は変更0 ID。`single` 6群は103〜106で先後各1戦、計12統制記録へ接続した。108の通常意思決定2戦、109の同名2枚6戦、110の盤面併用6戦は固定線completed記録で、独立した強度標本には数えない。113は入場監査、114はprotocol hardening、115はfirst-choice監査だけでcompleted 0件。101の3条件×6群は固定completed記録へ接続済みだが、全24戦を通常の独立対戦へ数えない。
