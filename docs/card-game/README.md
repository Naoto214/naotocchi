...
現在は **全カードプールの能力本文 第1稿（Phase C）で、メイン248枚の本文案が揃い、本文横断監査へ進む地点**。01〜24の基本ルール・8属性代表・A/B模擬・カブト／クワガタを基準に、未展開だった58枚を68〜70で具体化した。63〜66のユーザー確定事項、67の旧A/B修正は維持する。非アート部分と強度は未凍結。カード専用の最終イラストは最後に行う。

最新は[70 既存6代表の本文展開・56局面](70-representative-six-lines-completion.md)。各代表ファイルに48枚の全文を置き、既存6枚を除く新規42枚で、本文案206→248・未展開42→0へ進んだ。[68 ほし／？？？16枚](68-mystery-two-lines-card-text-draft.md)・[69 36局面](69-mystery-two-lines-text-audit.md)も保存済み。能力本文242枚＋能力なし6枚。裁定完備・大量模擬・強度確定を意味しない。
...
**次はメイン248枚の本文横断監査。** 70でいぬ②・かえる③・カクレクマノミ④の省略本文も完全化し、かえる①・カクレクマノミ①⑥⑦⑧の本編形態対応を整理した。旧数値・試行を保持。付け替えの誘発、コピーの有効先、れんくんの構築区分、準備札の低発動率・山札操作密度・強度を点検する。U05の0開始・誘発・性別と恋愛対象を使わない方針、65の途中終了、64の100維持期限・R10比較、63の通常行動は再質問しない。
...
1. メイン248枚の本文完備・重複・裁定を横断監査する。コピー・付け替え・効果追跡もここで再点検する。
2. なかま／こいびと／あいてむ／あそび／セカイ／できごとの本文設計。
3. 全体の重複・裁定・強度監査、プロキシによる大量テスト。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
4. 非アート部分を凍結。
5. 最後にカード専用イラスト。
