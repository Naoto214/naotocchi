...
現在は **[128 R2候補分類・ID・比較と独立再開](128-r2-candidate-extension.md)** の保存地点。127の4停止stateを原本とし、盤面能力分類、対象なしID、異種候補比較を独立の一般責務で接続した。後続の条件付きそだち効果の比較証明で4経路が真正停止し、completed・独立balance標本は0。
