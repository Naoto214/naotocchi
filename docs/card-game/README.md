...
現在は **全種類横断監査から再現可能な大量プロキシ準備へ進む地点**。最新は[97 全種類の役割密度・予約・時収支監査](97-role-density-and-efficiency-audit.md)。現行452件を重複可能な処理タグと個別本文で分類し、山札参照183、山札下145、予約らしい語形の和集合107、時1の取得／回収40、時1のそだち加算24を固定した。語句件数は発動回数ではない。32局面で同名積重ね、盤面得点エンジン、時1の補充＋そだち、安い選択取得、回収再使用、予約負荷を分け、今回はカード本文を変更していない。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更3 ID、96の4 ID改稿を維持し、97は変更0 ID。次は新12・旧24・HOLD14・legacy4の区分を保った採否比較と、版・山札順・予約ID・個体IDまで再現する大量プロキシ入力を作る。

97開始時の最新main実refも `f23398e43eed952d49d5a2b805789a4c87b2b1bd` で93〜96から変更なし。取得した6 sourceの実体・blobが93の記録と一致。本編は未マージ。93時点のc77587b→f23398e差分件数は、実一覧に基づき「7」から「6ファイル」へ訂正した（94 F94-01）。
...
3. 95の未接続4 IDは96で既存供給へ接続し、97で全452件の役割重複・山札操作／予約密度・時収支・反復効率を分類した。次は新12・旧24・HOLD14・legacy4の区分を保って採否を整理し、97の優先6群を再現可能なプロキシ入力へ落とす。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
4. 非アート部分を凍結。
5. 最後にカード専用イラスト。
