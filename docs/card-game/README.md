...
現在は **全種類横断監査の供給不足を個別改稿し始めた地点**。最新は[95 除去供給の第1次改稿](95-removal-supply-first-revisions.md)。りゅう⑥を時3・手札1・準備1で相手メイン／なかま1枚の捨て札除去へ、クワガタ⑥を他カードからの適正な引受へ、ボウリングを時3・印刷時上限2・対象1か2へ改稿した。32局面は机上確認。94の17 IDのうち13 IDへ少なくとも一つの実適用経路を接続したが、置換経由を含み、全条件の供給完備や強度確定ではない。ペンギン⑦は保護期限とりゅうの発動可能ターンが噛み合わず残件。メイン手札戻し、セカイ／こいびと捨て札行き等と全体の役割・効率監査を続ける。

現行カタログはメイン248・なかま26・こいびと18・あそび100・セカイ13・現行アイテム26・できごと21＝452件（能力なし7を含む）。これは登録477の完成枚数ではない。未登録アイテム12を含み、旧退役24・HOLD・legacyは別管理。95の本文変更は3 IDだけで、メイン数値・通常時・登録区分は維持。ボウリングの対象となる現行装備は印刷時2の8種。94の旧版では適正装備0だった履歴と、旧おはな等のlegacy試験を区別する。

95開始時の最新main実refも `f23398e43eed952d49d5a2b805789a4c87b2b1bd` で93保存時から変更なし。取得した6 sourceの実体・blobが93の記録と一致。本編は未マージ。93時点のc77587b→f23398e差分件数は、実一覧に基づき「7」から「6ファイル」へ訂正した（94 F94-01）。
...
**以下は91以前の再開メモ。できごとCARD21は91で本文接続済み、現行の供給監査は94を参照する。** 本編アイテム改良はユーザー連絡後に76〜78へ反映済み。旧24の採否・新12の正式登録・シール商品のカード化単位は区別して整理する。相手メイン除去・ゆきだるまの捨て札行き、73 V01・75 V02は大量プロキシ前に接続する。U05の性別・恋愛対象を使わない方針、63〜65の確定事項は再質問しない。
...
3. 95の残件（期限の噛合い、メイン手札戻し、こいびと／セカイ捨て札供給）を個別調整し、全種類の重複・裁定・強度監査、再現可能なプロキシによる大量テストへ進む。メイン横断監査第1周で残した密度・強度も測る。Aは現行08、Bの新しい試験は67の1枠改訂を識別して使い、旧試行と分ける。第0弾の選抜案も検証する。
4. 非アート部分を凍結。
5. 最後にカード専用イラスト。
