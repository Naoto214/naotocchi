...
| onStageChangedの通常成長・種別の姿変更 | NO-CARD | M-<species>-<01..08>の248形態と、たんじょう／ときとばし／へんしんが担当。各段階の通知を再加算しない。 |
...
カクレクマノミの体の変化・クエスチョニングの解消など、性別／恋愛対象に関わる本編分岐も確認した。これらはまず該当メイン・恋愛設計の資料として保持し、独立イベント候補への展開は保留する。今回の件数には追加していない。
