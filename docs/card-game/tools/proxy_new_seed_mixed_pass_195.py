...
import proxy_new_seed_followup_response_194 as audits
...
SOURCE_RAW_SHA256='31eaa8c09a39ca5e282c4b19bc206ff5f9bff6eb19162ee2a65f3ecc07a5c228'
OUTPUT=ROOT/'data/proxy-new-seed-mixed-pass-195-20260925.json'
SCHEMA='naotocchi.card_game.proxy_new_seed_mixed_pass_195.v1'
...
def load_sources():
    ...
    if hashlib.sha256(raw).hexdigest()!=SOURCE_RAW_SHA256 or \
        ...
        raise ValueError('195 protected audit/state raw differs')
    ...
    if proof['schema']!=audits.SCHEMA or original['schema']!=states.SCHEMA or \
        ...
        raise ValueError('195 current response candidates differ')
...
def run_route(row,proof):
    if (row['path_id'],row['last_valid_event_seq'],row['final_game_state_sha256'],
        ...
        raise ValueError('195 source state/audit boundary differs')
    ...
    if start._hash(state)!=state['continuation_state_sha256']:
        raise ValueError('195 current continuation differs')
    ...
    for excluded in proof['board_exclusions']:
        ...
        else:raise ValueError('195 projected board exclusion identity differs')
    ...
    if chance['legal_candidate_ids']!=proof['candidate_ids'] or \
        ...
        raise ValueError('195 current pass completeness differs')
    ...
    if decision['selected_candidate']!='response-pass' or \
        ...
        raise ValueError('195 unique pass selection differs')
    ...
    if phase=='post_placement_response':
        ...
        if transitioned['chain_status']!='resolving' or \
            ...
            raise ValueError('195 chain reverse resolution differs')
    ...
    else:raise ValueError('195 current phase unclassified')
...
def validate_result(result):
    try:
        ...
        if result!=run_route(row,proof) or result['last_valid_event_seq']!=row['last_valid_event_seq']+1:
            return ['195 independent replay differs']
        ...
        if event['seq']!=result['last_valid_event_seq'] or \
            ...
            return ['195 event/snapshot/hash chain differs']
        return []
...
def build_report():
    ...
    if len(rows)!=4 or any(validate_result(x) for x in rows):
        raise ValueError('195 mixed response replay differs')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes()!=raw:raise SystemExit('195 canonical bytes differ')
    else:OUTPUT.write_bytes(raw)
    print('195: 4 unique passes; one ability chain resolving, two turn ends')
