...
SOURCE_RAW_SHA256='44c5ef43c10d683e0777dd727cb55df49f1be835cfdedfb973da172fe22d98bc'
BASELINE_RAW_SHA256='8621db1d8b7d003399ce8858ed19a8b53921ac85381bc42b962049dc2e1e7318'
OUTPUT=ROOT/'data/proxy-new-seed-turn-end-proof-203-20260925.json'
SCHEMA='naotocchi.card_game.proxy_new_seed_turn_end_proof_203.v1'
SOURCE_REFS={
...
def load_sources():
    ...
    if hashlib.sha256(raw).hexdigest()!=SOURCE_RAW_SHA256 or \
        ...
        raise ValueError('203 protected 202/184 raw/replay differs')
    ...
    if current['schema']!=states.SCHEMA or history['schema']!=baseline.SCHEMA or \
        ...
        raise ValueError('203 saved state/history inventory differs')
    return current,history


def audit_route(row,old):
    path=row['path_id'];state=row['final_continuation_state']
    if old['path_id']!=path or state['game_state']['phase']!='turn_end' or \
            state['return_target']!='turn_end':
        raise ValueError('203 current turn end boundary differs')
    ...
    event_counts={}
    for number in range(185,203):
        files=list((ROOT/'data').glob(f'proxy-new-seed-*-{number}-*.json'))
        if len(files)!=1:raise ValueError('203 checkpoint history file inventory differs')
        ...
        if not isinstance(events,list) or not isinstance(shots,list) or len(events)!=len(shots):
            raise ValueError('203 checkpoint event/snapshot inventory differs')
        ...
        for event,snap in zip(events,shots):
            ...
            if action not in SOURCE_REFS or event['seq']!=seq+1 or \
                ...
                raise ValueError('203 historical event/snapshot/hash chain differs')
            ...
            if any(delta.values()) or snap['continuation_state']['pending_triggers'] or \
                ...
                raise ValueError('203 historical growth/pending state differs')
    ...
    if (seq,game_sha,cont_sha)!=(row['last_valid_event_seq'],
        ...
        raise ValueError('203 final state/history boundary differs')
    ...
    if not result['turn_end_set_complete'] or result['contract_stop_codes'] or \
        ...
        raise ValueError('203 six-stage turn end incomplete')
...
def validate_result(result):
    try:
        ...
        return [] if result==audit_route(row,old) else ['203 independent provenance differs']
...
def build_report():
    ...
    if len(rows)!=4 or any(validate_result(x) for x in rows):
        raise ValueError('203 four historical six-stage ends differ')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes()!=raw:raise SystemExit('203 canonical bytes differ')
    else:OUTPUT.write_bytes(raw)
    print('203: four complete historical six-stage turn ends')
