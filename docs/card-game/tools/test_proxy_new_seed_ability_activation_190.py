import copy
import unittest

import proxy_new_seed_ability_activation_190 as subject


class AbilityActivation190Test(unittest.TestCase):
    def test_three_unique_passes_one_board_activation(self):
        report=subject.build_report()
        self.assertEqual(4,report['new_events'])
        for row in report['results']:
            self.assertEqual([],subject.validate_result(row))
            self.assertEqual(0,row['balance_sample_count'])
        activated=[x for x in report['results'] if x['new_events'][0]['action_type']=='activate_response']
        self.assertEqual(1,len(activated))
        self.assertEqual('board',activated[0]['final_continuation_state']['activation_zone'][0]['source_zone'])

    def test_modified_event_hash_is_rejected(self):
        row=copy.deepcopy(subject.build_report()['results'][0])
        row['new_events'][0]['game_state_after_sha256']='0'*64
        self.assertTrue(subject.validate_result(row))


if __name__=='__main__':unittest.main()
