#!/usr/bin/env python3
"""Apply the existing seeded egg choice to all four next turns."""
...
import proxy_new_seed_turn_end_restart_164 as prior
...
SOURCE = prior.OUTPUT
SOURCE_RAW_SHA256 = '20c8b40b5f347cff3a6be7dc3625b1afacef2c422b1f63c551ecd9178d1ec73b'
OUTPUT = ROOT / 'data/proxy-new-seed-egg-restart-165-20260925.json'
SCHEMA = 'naotocchi.card_game.proxy_new_seed_egg_restart_165.v1'
...
def load_source():
    ...
    if hashlib.sha256(raw).hexdigest() != SOURCE_RAW_SHA256 or raw != prior.canonical_bytes(prior.build_report()):
        raise ValueError('164 protected raw or replay differs')
    result = json.loads(raw)
    if result['schema'] != prior.SCHEMA or any(prior.validate_result(x) for x in result['results']):
        raise ValueError('164 source chain differs')
    return result
...
def run_route(row):
    ...
    if start._hash(state) != state['continuation_state_sha256'] or \
            ...
            state['game_state']['round'] not in (1,2):
        raise ValueError('164 current egg state differs')
    game = state['game_state']; actor = game['turn_player']; player = game['players'][actor]
    if len(player['hand']) < 2 or player['reservations'] or state['activation_zone'] or state['pending_triggers']:
        raise ValueError('165 mandatory egg choice boundary differs')
    ...
    decision = opening.build_mandatory_choice_decision({'order_id': order}, actor, 1,
                                                        game['round'], hand)
...
def validate_result(row):
    try:
        ...
        if row != run_route(original) or row['last_valid_event_seq'] != original['last_valid_event_seq'] + 1:
            return ['165 independent replay differs']
        ...
        if event['seq'] != row['last_valid_event_seq'] or \
            ...
            return ['165 event/snapshot hash chain differs']
        return []
...
def build_report():
    ...
    if len(rows) != 4 or any(validate_result(row) for row in rows):
        raise ValueError('165 egg replay differs')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes() != raw: raise SystemExit('165 saved canonical bytes differ')
    else: OUTPUT.write_bytes(raw)
    print('165: 4 seeded egg exchanges, 4 events, 0 independent balance samples')
