...
SOURCE_RAW_SHA256='f5ed4157ee44558b73303e08a114ad70fda40e576e290cfa18e674aa38add0fd'
BASELINE_RAW_SHA256='8621db1d8b7d003399ce8858ed19a8b53921ac85381bc42b962049dc2e1e7318'
OUTPUT=ROOT/'data/proxy-new-seed-turn-end-proof-219-20260925.json'
SCHEMA='naotocchi.card_game.proxy_new_seed_turn_end_proof_219.v1'
SOURCE_REFS={
...
def load_sources():
    ...
    if hashlib.sha256(raw).hexdigest()!=SOURCE_RAW_SHA256 or \
        ...
        raise ValueError('219 protected 202/184 raw/replay differs')
    ...
    if current['schema']!=states.SCHEMA or history['schema']!=baseline.SCHEMA or \
        ...
        raise ValueError('219 saved state/history inventory differs')
...
def audit_route(row,old):
    ...
    if old['path_id']!=path:
        raise ValueError('219 baseline path differs')
    if row['stop_reason_code']!='unproved_current_turn_end_provenance':
        if state['game_state']['phase'] not in ('normal_action','post_placement_response') or \
                row['new_events']==0 or len(row['new_events'])!=1:
            raise ValueError('219 held route boundary differs')
    ...
    if state['game_state']['phase']!='turn_end' or state['return_target']!='turn_end':
        raise ValueError('219 current turn end boundary differs')
    ...
    event_counts={}
    for number in range(185,219):
        files=list((ROOT/'data').glob(f'proxy-new-seed-*-{number}-*.json'))
        if len(files)!=1:raise ValueError('219 checkpoint history file inventory differs')
        ...
        if not isinstance(events,list) or not isinstance(shots,list) or len(events)!=len(shots):
            raise ValueError('219 checkpoint event/snapshot inventory differs')
        ...
        for event,snap in zip(events,shots):
            ...
            if action not in SOURCE_REFS or event['seq']!=seq+1 or \
                ...
                raise ValueError('219 historical event/snapshot/hash chain differs')
            ...
            if any(delta.values()) or snap['continuation_state']['pending_triggers'] or \
                ...
                raise ValueError('219 historical growth/pending state differs')
    ...
    if (seq,game_sha,cont_sha)!=(row['last_valid_event_seq'],
        ...
        raise ValueError('219 final state/history boundary differs')
    ...
    if not result['turn_end_set_complete'] or result['contract_stop_codes'] or \
        ...
        raise ValueError('219 six-stage turn end incomplete')
...
def validate_result(result):
    try:
        ...
        return [] if result==audit_route(row,old) else ['219 independent provenance differs']
...
def build_report():
    ...
    if len(rows)!=4 or sum(x['turn_end_set_complete'] for x in rows)!=2 or \
            any(validate_result(x) for x in rows):
        raise ValueError('219 four historical six-stage ends differ')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes()!=raw:raise SystemExit('219 canonical bytes differ')
    else:OUTPUT.write_bytes(raw)
    print('219: two complete historical six-stage turn ends, two held')
