#!/usr/bin/env python3
"""Close three proven end-request windows; keep the fourth turn end intact."""
...
import proxy_new_seed_normal_restart_160 as prior
...
SOURCE = prior.OUTPUT
SOURCE_RAW_SHA256 = '5737267da095d2d56690f7a07f9da0cb32a89f614d396d8e3014ef33416a2ed0'
OUTPUT = ROOT / 'data/proxy-new-seed-end-response-restart-161-20260925.json'
SCHEMA = 'naotocchi.card_game.proxy_new_seed_end_response_restart_161.v1'
...
def load_source():
    ...
    if hashlib.sha256(raw).hexdigest() != SOURCE_RAW_SHA256 or raw != prior.canonical_bytes(prior.build_report()):
        raise ValueError('160 protected raw or replay differs')
    data = json.loads(raw)
    if data['schema'] != prior.SCHEMA or any(prior.validate_result(x) for x in data['results']):
        raise ValueError('160 state/hash chain differs')
    return data
...
def current(row):
    ...
    if start._hash(state) != state['continuation_state_sha256']:
        raise ValueError('160 continuation source SHA differs')
    return state
...
def opportunity(state, origin):
    ...
    if ctx['window_kind'] != 'after_normal_action' or ctx['phase'] != 'response_window' or \
        ...
        raise ValueError('161 current response boundary differs')
    board = game['players'][actor]['board']
    if board['prepared'] or board['main'] is not None or board['world'] is not None:
        raise ValueError('161 unproved board response source')
    ...
    for instance in board['companions']:
        ...
        if any(fragment not in section for fragment in fragments) or \
            ...
            raise ValueError('161 companion response timing not excluded')
    ...
    if instance is not None:
        ...
        if condition not in section or '発動できる' not in section or \
                origin['seq'] != ctx['origin_event_seq'] or \
                origin['action_type'] != 'normal_pass_end_request':
            raise ValueError('161 partner response timing not excluded')
        # A normal pass cannot be a challenge or world change.
        exclusions.append({'source_instance_id':instance,'card_id':card_id,
                           'reason_code':'trigger_condition_not_met'})
    ...
    if opportunity['legal_candidate_ids'] != ['response-pass'] or not opportunity['candidate_set_complete']:
        raise ValueError('161 response candidate not uniquely proved')
    return opportunity
...
def run_route(row):
    ...
    if phase not in ('turn_end_response','turn_end') or state['return_target']!='turn_end':
        raise ValueError('161 unclassified end response phase')
    order = next(x for x in start.load_source()['results'] if x['path_id']==row['path_id'])['order_id']
    for index in range(1 if phase=='turn_end_response' else 0):
        if len(row['new_events'])!=1 or row['new_events'][-1]['action_type']!='normal_pass_end_request' or \
                state['response_context']['origin_event_seq']!=row['new_events'][-1]['seq']:
            raise ValueError('161 origin event differs')
        ...
        if decision['selected_candidate']!='response-pass' or decision['resolution_mode']!='response_unique':
            raise ValueError('161 response pass not unique')
        ...
        if state['response_context']['consecutive_passes']!=1 or \
            ...
            raise ValueError('161 end-request closing priority differs')
...
def validate_result(result):
    try:
        ...
        if result!=run_route(source) or result['last_valid_event_seq']!= \
            ...
            return ['161 independent replay differs']
        ...
        for offset,(event,snap) in enumerate(zip(result['new_events'],result['new_snapshots']),1):
            if event['seq']!=source['last_valid_event_seq']+offset or \
                ...
                return ['161 event/snapshot hash chain differs']
...
def build_report():
    ...
    if len(rows)!=4 or any(validate_result(x) for x in rows):
        raise ValueError('161 independent response replay differs')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes()!=raw:raise SystemExit('161 saved canonical bytes differ')
    else:OUTPUT.write_bytes(raw)
    print('161: 3 end response passes, 4 turn ends, 0 completed')
