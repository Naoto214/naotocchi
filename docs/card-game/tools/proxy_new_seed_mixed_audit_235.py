...
SOURCE_RAW_SHA256='ec0321a67416a27a666243d8aafc92cdd95d40cb2e1bf0111e2f204db6b837a1'
OUTPUT=ROOT/'data/proxy-new-seed-mixed-audit-235-20260925.json'
SCHEMA='naotocchi.card_game.proxy_new_seed_mixed_audit_235.v1'
...
def load_source():
    ...
    if hashlib.sha256(raw).hexdigest()!=SOURCE_RAW_SHA256 or \
        ...
        raise ValueError('235 protected 234 raw/canonical differs')
    source=json.loads(raw)
    if len(source['results'])!=4 or any(states.validate_result(x) for x in source['results']):
        raise ValueError('235 source state integrity differs')
    return source
...
def audit_route(row):
    ...
    if start.canonical_sha256(state)!=row['final_continuation_state_sha256'] or \
        ...
        raise ValueError('235 source state/hash differs')
    ...
    if row['stop_reason_code']=='unproved_current_normal_action_candidates':
        ...
        if proof['next_opportunity']!='normal_action' or \
            ...
            raise ValueError('235 normal opportunity incomplete')
    ...
    if row['stop_reason_code']!='unproved_current_egg_exchange_choice' or \
        ...
        raise ValueError('235 mandatory egg boundary differs')
    ...
    if fallback.validate_seeded_resolution(decision) or \
        ...
        raise ValueError('235 mandatory candidate completeness differs')
...
def validate_result(result):
    try:
        ...
        return [] if result==audit_route(row) else ['235 independent audit differs']
...
def build_report():
    ...
    if len(rows)!=4 or sum(x['next_opportunity']=='mandatory_egg_exchange' for x in rows)!=3 or \
        ...
        raise ValueError('235 mixed opportunities differ')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes()!=raw:raise SystemExit('235 canonical bytes differ')
    else:OUTPUT.write_bytes(raw)
    print('235: three egg exchanges and one normal opportunity complete')
