...
SOURCE_RAW_SHA256='fe26a9495581084751061feee8d23bed92d4fda19be5f3e73cf375f5f5dd375e'
OUTPUT=ROOT/'data/proxy-new-seed-followup-audit-213-20260925.json'
SCHEMA='naotocchi.card_game.proxy_new_seed_followup_response_213.v1'
...
def audit_route(row):
    ...
    if phase not in ('post_placement_response','response_window','turn_end_response') or \
        ...
        raise ValueError('213 current response boundary differs')
    if phase=='post_placement_response' and \
            (origin['action_type']!='place_partner' or ctx['consecutive_passes']!=0):
        raise ValueError('213 post-placement origin differs')
    if phase=='turn_end_response' and \
        ...
        raise ValueError('213 turn-end response origin differs')
    if phase=='response_window' and \
        ...
        raise ValueError('213 chain response origin differs')
    ...
    for instance in owner['hand']:
        ...
        if entry is None:raise ValueError('213 missing hand candidate table')
        ...
        if exclusion is None and card_id=='G-animal-shogi':
            ...
            if '自分の捨て札のなかま1枚を対象' not in section or \
                ...
                raise ValueError('213 shogi target requires separate proof')
    ...
    for instance in board_state['companions']:
        ...
        if any(x not in section for x in fragments) or \
            ...
            raise ValueError('213 companion response cannot be excluded')
    ...
    if partner is not None:
        ...
        if card_id=='P-cat_ceo':
            if '交際を始めた時、発動する' not in section or \
                ...
                raise ValueError('213 egg suppression proof differs')
            reason='partner_ability_disabled_during_egg'
        elif card_id=='P-cliff_goat' and '初配置・同名上書き' in section and \
                board_state['world'] is None and origin['action_type'] in \
                ('normal_pass_end_request','place_partner'):
        ...
        else:raise ValueError('213 partner ability timing unclassified')
    ...
    if chance['legal_candidate_ids']!=['response-pass'] or \
        ...
        raise ValueError('213 response candidate completeness differs')
...
def validate_result(result):
    try:
        ...
        return [] if result==audit_route(row) else ['213 independent audit differs']
...
def build_report():
    ...
    if len(rows)!=4 or any(validate_result(x) for x in rows):
        raise ValueError('213 mixed response opportunities differ')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes()!=raw:raise SystemExit('213 canonical bytes differ')
    else:OUTPUT.write_bytes(raw)
    print('213: four complete mixed response windows; each unique pass')
