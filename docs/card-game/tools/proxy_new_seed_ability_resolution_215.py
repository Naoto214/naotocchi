...
import proxy_new_seed_followup_replay_214 as prior
...
SOURCE=prior.OUTPUT
SOURCE_RAW_SHA256='c0ee638060979a1f19daa4c4ea5afb2e89f1425819255635b232234fa236c747'
OUTPUT=ROOT/'data/proxy-new-seed-ability-resolution-215-20260925.json'
SCHEMA='naotocchi.card_game.proxy_new_seed_ability_resolution_215.v1'
...
def resolve_board_ability(before):
    ...
    if before['game_state']['phase']!='response_window' or ctx['chain_status']!='resolving' or \
        ...
        raise ValueError('215 unproved ability resolution chain')
    ...
    if link['action_type']!='activate_board_ability' or link['source_zone']!='board' or \
        ...
        raise ValueError('215 reached C-chicken board source differs')
    ...
    if '山札上1枚を公開する。それがなかまカードなら、手札に加える' not in text or \
        ...
        raise ValueError('215 C-chicken approved effect differs')
    ...
    if registered is None or registered['card_type'] not in start.VARIANTS:
        raise ValueError('215 revealed card category unregistered')
...
def run_route(row):
    ...
    if start._hash(state)!=state['continuation_state_sha256']:
        raise ValueError('215 source continuation hash differs')
    ...
    else:raise ValueError('215 held route differs')
...
def validate_result(result):
    try:
        ...
        if result!=run_route(source) or result['last_valid_event_seq']!=source['last_valid_event_seq']+len(result['new_events']):
            return ['215 independent replay differs']
        ...
        for event,snap in zip(result['new_events'],result['new_snapshots']):
            if event['game_state_before_sha256']!=game or event['continuation_state_before_sha256']!=cont or \
                ...
                return ['215 event/snapshot hash chain differs']
...
def build_report():
    ...
    if len(rows)!=4 or any(validate_result(x) for x in rows):
        raise ValueError('215 board ability replay differs')
...
def main():
    ...
    if args.check:
        if OUTPUT.read_bytes()!=raw:raise SystemExit('215 canonical bytes differ')
    else:OUTPUT.write_bytes(raw)
    print('215: board ability revealed top, noncompanion retained; 0 completed')
