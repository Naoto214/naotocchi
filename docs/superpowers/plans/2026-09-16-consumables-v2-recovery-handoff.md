...
> **2026-09-16 現在の継続位置:** この文書の本文は復旧開始時の履歴。使い切りTask5/6を含む12種は[使い切り完成・検証記録](2026-09-16-consumables-v2-verification.md)、続く承認済み通常装具10＋使い切り12・Quick／日次／イベント経済・伝説は[最新の経済検証記録](2026-09-16-items-v2-economy-verification.md)を先に読む。「Task4まで」「Quick等は対象外」は当時の範囲で、再実装の指示ではない。なおとシリーズ4達成品の新効果は別枠の[未承認設計](../specs/2026-09-16-naoto-series-v2-open-decisions.md)。PRはDraft・未マージを維持し、正確な最新remote HEAD／CIは最終検証記録を確認する。
