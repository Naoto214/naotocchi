...
### Task 4: Keep conversation layout fixtures stable during the real paper cooldown

**Evidence:** Published `eeb8746390b454f4905bc24bfbfbcafd1739be00` is mergeable against main14e0b2a. Runtime smoke job104069623815 passed540/540. Home layout job104069622860 now passes all690 source glyphs in both Chromium and WebKit, but paper-equipped conversation scenarios with three or four poops fail `speaking moves poop`: the remaining rectangles have identical coordinates and the last rectangle disappears. Conversation clocks advance beyond the normal3-second activity tick; the approved paper effect removes one poop at count>=3 and then waits60 activity ticks. Two-poop, no-equipment, flower and crown cases do not have this failure. This is a fixture/gameplay interaction, not evidence to weaken the fixed-layout contract.

**Files:** Only `tests/home-conversation-browser.cjs` fixture preparation and factual QA chronology. Use the existing `item-system.js` cooldown API to start paper-equipped specimens in their legitimate60-tick waiting period after all equipment overrides. Preserve every scenario, viewport, clock advance, poop count, geometry assertion, conversation action, manual-clean assertion and real accessory. Do not change production effects, item prices, source assets, workflow, timeout or assertion thresholds. No new production flag or broad helper abstraction.

**Interfaces:** `NaotocchiItems.cooldown(save, 'paper', 60)` normalizes the saved item state and sets the deadline relative to existing activity ticks. This is a valid saved state, not a permanent disable. The game must keep the fixture's poop count during the conversation and resume one-poop automatic assistance when the real cooldown expires. Existing paper runtime tests remain the behavior authority.

- [x] **Step 1:** Confirm the CI failure and source contract. Capture a focused Node runtime comparison of an unreserved paper specimen against an otherwise equal specimen prepared using the public cooldown API. Check reload retains the deadline, the conversation's short tick window preserves its poop count, and assistance resumes at the real60-tick boundary without care/growth reward. Record original and prepared results; do not run a browser or render CI artifacts.
- [x] **Step 2:** Apply the smallest fixture-only initialization after equipment overrides, with a short explanation of why layout measurements need the ordinary cooldown. Run `node --check tests/home-conversation-browser.cjs` and existing focused paper/cooldown runtime coverage. Keep all browser assertions unchanged. Do not add a test that merely matches source text or duplicate the entire fixture builder.
- [x] **Step 3:** Commit the fixture correction; record exact command/output and source SHA in QA, followed by a documentation-only commit. The unchanged production tree already has local540/540 onfbd3068 and published540/540 oneeb8746. A duplicate local full run is unnecessary for this browser-fixture-only change; controller requires the naturally triggered Runtime smoke and Home layout CI on the new published commit before Ready.
- [x] **Step 4:** Self-review and report BASE/HEAD, changed files, failing CI evidence, complete focused test/probe outputs, preserved assertions and limits in this plan workspace `task-4-report.md`. No external action or subagents. Controller performs a fresh scoped review, publishes a normal descendant ofeeb8746 on the same feature branch, then observes existing CI without manually dispatching or rerunning it. Earlier economic comparison is unaffected and must retain its actual revision label.
...
Execution note: Tasks1–3 and their scoped reviews are complete. Current product/test/cache gate is540/540 onfbd3068, also confirmed by the published Runtime smoke CI oneeb8746. Task2 bounds follow-up was covered by focused41/41 and the finalTask3 fullgate after the necessaryPR269 integration; the earlier53b3651 gate remains historical. Task4 and its fresh scoped review are complete: fixture sourceb08208a,39/39 focused tests, syntax and real cooldown probe passed; no review findings. Both naturally triggered CI jobs on the next published commit remain the Ready gate. Controller owns PRpublication, automaticCI observation andReady verification.
