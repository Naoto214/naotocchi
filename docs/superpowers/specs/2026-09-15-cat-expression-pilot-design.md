...
### れんくん（ren）80表情の継続制作（2026-09-23）

unknown保存HEAD `570673e51d0550721c0762e10420d5683adba19a` / tree `024909b1f2bfa25ee8af05ca8fa9007ff7912d71` を基準に、マスターのnormal22・rare8が完成済みでsecret renが未制作であること、8段階の成長文・全8元画像からrenを選定。テストfallbackを選定根拠にしていない。主担当・独立担当が全8原寸/6倍を確認し、同じ一人が乳幼児から高齢へ成長する単一身体・単一顔で、新たなA/B/C分類の曖昧さはない。

ちびれん→げんきれん→子どもれん→少年れん→若者れん→大人れん→年を重ねたれん→おじいちゃんれんの年齢・体格・髪型/白髪・姿勢・衣服を保持する。01のおしゃぶりは外さず目/眉を中心に表現し、隠れた口を新しく描かない。02胸のクマ顔風の図柄は衣服のプリントであり、生体や表情同期対象にしない。03のユニフォームと番号・サッカーボール、04/05リュック、06鞄、08杖と加齢の顔線を不調/睡眠でも保持。身体ポーズを変えず、元の閉じ目/ウィンクは必要に応じ表情の目へ変更する。新しい涙・汗・状態マークはPNGへ追加しない。

既存30系統2400表情＋旧4＋通常297＝2701PNG、240配置、30anchors/namesを保持基準とする。80個別制作/provenance/SHA/全数独立目視/独立合成/新8配置のみ追加/保持・全接続・全配置・全体テスト/同一owner-private Site更新/Git Data API保存までを一区切りとする。PR278 Draft/open/未マージ、main取り込み・競合解消なし。既存3分類とunknown04の正体不明器官裁定、resolver・マーク色/形/配置制約/例外・ゲーム数値・成長/恋愛条件・セーブ互換を保持する。分類曖昧さ以外の段階再承認は不要。今回保存後に次へ自動進行しない。

ren80の個別制作・全数独立PNG/合成受理・保持/接続/配置・全体1838PASS・同一owner-private Site71公開を完了。詳細は `docs/qa/ren-expressions-20260923.md` とmanifest/review群を正とする。既存2400表情を保持し31系統2480表情。全キャラクター制作完了とは扱わず、今回のGitHub保存後に区切る。保存HEAD/tree/PR/CIはPR278最終追記に記録する。


### 通常育成31系統の最終横断監査（2026-09-23）：完成保留

現GitHub保存tree `d0399d249c82dd74159fdd8ac592e0d1bd405204` を一次資料として、normal22＋rare8＋ren1、計31系統・248段階・2480表情を独立再計数し全数目視した。2480現役＋履歴保持4の完全性、通常297PNGの制作前hash不変、runtime2728描画・全体1838テストはfreshに確認したが、これを完成判定に代用しない。

現在の総合判定は **NOT GREEN / 修正方針の確認待ち**。dog03-wantsPlayの前足姿勢、antlion07の5状態の軌跡、antlion08の計画/成長文/原画保持の不整合、starfish08の小泡、種別空腹マークの個別意図不足、病気と非sick表情の同時表示時の汗対マーク衝突を監査記録へ保存する。A/B/C分類とunknown04の正体不明器官裁定は保持し、新たな分類曖昧さは認めない。

今回のユーザー指定停止条件（複数の妥当な修正でキャラクター設定が変わる場合は修正前確認）に従い、ゲーム実装・既存PNG・マークを修正しない。新規生成・再生成0。確認Siteは監査結果と比較根拠を追加する。詳細/期待実数/全所見/最小修正対象/検証範囲・限界は `docs/qa/final-cross-audit-20260923.md` と関連証拠群を現行監査正本とする。過去の個別受理記録は履歴として保持し、今回の指摘を無かったことにしない。

なおと・なかま・こいびとの表情制作を開始しない。PR278はDraft/open/未マージ、main取込み・マージ・競合解消なし。必要な設定判断後に最小修正・独立受理・fresh検証・Site/GitHub保存が完了するまで、通常育成表情編を完成扱いにしない。


### 空腹絵柄・位置の人間最終裁定（2026-09-26 22:55 JST）

**設計決定済み／本番接続・位置実装は未実施。** 最新の人間判断として以下を正とし、旧比較の推奨・人間判断待ち表記は比較時点の履歴とする。

- 食事15絵柄は確定。中立養分はA「光の核＋途切れた環」、底生小餌はA「二枚貝＋巻貝＋小片＋海底線」。他13絵柄も人間確認済み改善候補を維持し、再設計しない。
- 通常31系統248段階の空腹位置は **247段階現状維持／キノコ07のみB採用**。
- キノコ07「粉を飛ばすキノコ」はhungryのoffsetを、将来実装時に `[-5,-1]` → **`[-23,-9]`** とする（104座標系、左18・上8）。別個体の胞子列と空腹バブルの視覚的混同を避けるためであり、思考バブルを頭上へ統一するためではない。他状態のoffset、汗、画像はこの裁定で変更しない。
- 前回9段階（おとこのひと01/04/05/06/07/08、クワガタムシ01、フェニックス03/07）は全てA現状維持。おとこのひと01・クワガタムシ01への旧B推奨は採用しない。
- **「口元寄り＝不具合」「頭上ほど正しい」としない。** 空腹マークは思考に加え、「食べたい」「お腹すいた」という欲求表現。赤ちゃん・幼体・幼虫等では、口元→小バブル→食物の近さが自然で可愛い場合がある。逆に全てを口元へ揃えず、植物・菌類・群体・特殊形態は全体との視覚的つながりを優先する。差が小さい場合は現状維持。
- 同一身体・一群・別個体の裁定を保持し、別個体の胞子を空腹表示の主役にしない。思考丸や食物は表情PNGへ焼き込まない。本体→汗→状態マーク、代表状態マーク1種類、semantic default＋必要段階overrideを維持。
- ヒトデ新01〜03の正式反映後は新身体に対する限定位置QAが必要。今回の旧runtime画像での位置監査を新画像の検証済み根拠に流用しない。

証拠：[248段階追加監査](../../qa/hunger-position-appetite-review-20260926.md)。実装前の現状・安全な順序は[実装チェックリストの最新追記](../../qa/growth-hunger-expression-implementation-checklist-20260926.md)を参照。この保存区切りは文書のみで、次工程の実装・画像修正・mainマージへ進まない。


## 2026-09-26 手順1：食事マークの本番接続

人間承認に基づき、19意味カテゴリを承認済み15 SVGへ接続する。意味割当は既存hungerCategoryForの系統default＋必要段階overrideを維持し、248/248一致を確認。中立養分A「光の核＋途切れた環」、底生小餌A「二枚貝＋巻貝＋小片＋海底線」、他13絵柄は候補からそのまま移植。

キノコ07のhungryだけ承認B `[-23,-9]` を反映。別個体の胞子列との混同回避であり、頭上へ統一するためではない。他247段階・前回9段階はA現状維持。「口元寄り＝不具合」「頭上ほど正しい」としない原則を維持する。

[手順1の検証記録](../../qa/hunger-runtime-step1-20260926.md)を参照。関連924件PASS、全体1,844件中4件は開始時からの名称同期不整合でFAIL。実ブラウザーHomeは環境制限により未確認。実装を保存するが手順1の全検証完了・最終GREENとはせず、手順2以降へ進まない。quick-mode初回FAILは今回全体内PASSでも原因未特定の追跡を継続する。
