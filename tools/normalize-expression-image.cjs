...
const original=await sharp(`${root}/assets/characters/${line}/${stage}.png`).ensureAlpha().raw().toBuffer({resolveWithObject:true});
let x0=128,y0=128,x1=0,y1=0;for(let y=0;y<128;y++)for(let x=0;x<128;x++)if(original.data[(y*128+x)*4+3]>0){x0=Math.min(x0,x);y0=Math.min(y0,y);x1=Math.max(x1,x);y1=Math.max(y1,y);}
