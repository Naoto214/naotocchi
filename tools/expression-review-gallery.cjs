...
(async()=>{
 for(const species of (process.argv[3]?process.argv[3].split(','):Object.keys(names))) {
  ...
  for(const start of [1,5]){
   const end=start+3;let header=`<svg xmlns="http://www.w3.org/2000/svg" width="1408" height="3340"><style>text{font-family:'M PLUS Rounded 1c',sans-serif;fill:#304341}</style><rect width="1408" height="3340" fill="#edf3ef"/><text x="24" y="42" font-size="30">${({cat:'猫',dog:'犬',man:'人間（男）',woman:'人間（女）',penguin:'ペンギン',turtle:'カメ',frog:'かえる',clownfish:'カクレクマノミ',salmon:'さけ',hermit_crab:'ヤドカリ',jellyfish:'クラゲ'})[species]} ${start}〜${end}段階｜表情・マーク調整後</text><text x="24" y="73" font-size="18">ゲーム素材・座標の静止合成（2倍表示）。実機撮影ではありません。</text>`;
   const parts=[];
