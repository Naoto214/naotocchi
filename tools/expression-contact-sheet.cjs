...
function buildSheet(species,stage,title){
 ...
 const cells=rows.map(([name,label],i)=>{
  ...
  const x=24+(i%2)*356,y=120+Math.floor(i/2)*256;
  // 104px logical canvas, uniformly magnified 2x. Retain production floor shift.
  const sweatY=Math.max(1,Math.min(104-15.6-Math.min(7,104*(b[3]-b[1])/128*.18)-3,104*b[1]/128+shift+104*(b[3]-b[1])/128*.16));
  const sweat=name==='sick'?[104*b[0]/128-10.4*.7,104-Math.max(1,104*(128-b[2])/128-10.4*.7)-10.4].map(sx=>`<ellipse cx="${Math.max(1,sx)+5.2}" cy="${sweatY+7.8}" rx="5.2" ry="7.8" fill="#a5d934" stroke="#689d15" stroke-width="1" opacity=".85"/>`).join(''):'';
  return `<g transform="translate(${x} ${y})"><rect width="340" height="240" rx="16" fill="#fff" stroke="#d6dedb"/><text x="18" y="30" font-size="19" fill="#304341">${i+1}. ${label}</text><g transform="translate(66 24) scale(2)"><image x="0" y="${shift}" width="104" height="104" image-rendering="optimizeSpeed" href="data:image/png;base64,${png}"/><g class="pet-expression-accent">${accent}</g>${sweat}</g></g>`;
 }).join('');
 return `<svg xmlns="http://www.w3.org/2000/svg" width="736" height="1480" viewBox="0 0 736 1480"><style>${style}text{font-family:'M PLUS Rounded 1c',sans-serif}</style><rect width="736" height="1480" fill="#edf3ef"/><text x="24" y="46" font-size="28" fill="#263d38">${escape(title)}｜10表情とマーク</text><text x="24" y="81" font-size="16" fill="#536c63">ゲームの画像・色・マーク座標を使用 ／ 2倍表示</text>${cells}<text x="24" y="1420" font-size="15" fill="#536c63">実機のスクリーンショットではなく、位置関係の確認用です。</text><text x="24" y="1450" font-size="15" fill="#536c63">背景・動き・画面幅は省略。病気の汗は静止した近似表示です。</text></svg>`;
}
