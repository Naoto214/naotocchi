...
const FORMS = Object.freeze({adult:25,kitten:7,otemba:12});
...
<body>
  <header data-preview-controls>
    ...
    <nav aria-label="猫の状態"><select data-preview-form aria-label="ねこの姿"><option value="adult" ${form==='adult'?'selected':''}>大人のねこ</option><option value="kitten" ${form==='kitten'?'selected':''}>こねこ</option><option value="otemba" ${form==='otemba'?'selected':''}>おてんばねこ</option></select>${links}</nav>
  </header>
  ...
  (() => {
    ...
    document.querySelector('[data-preview-form]')?.addEventListener('change',event=>{
      ...
      if (!['adult','kitten','otemba'].includes(form)) return;
...
if (require.main === module) {
  ...
  if (!outputPath) {
    process.stderr.write('Usage: node tools/cat-expression-preview.cjs <output.html> [preset] [adult|kitten|otemba]\n');
    process.exitCode=1;
  } else {
