...
const STAGED_FORMS = Object.freeze(Object.fromEntries(['man','woman','penguin','turtle','frog','clownfish','salmon','hermit_crab','jellyfish','starfish','coral','butterfly','beetle','stagbeetle','cicada','antlion','dandelion','sakura','venus_flytrap','mushroom','dragon','phoenix','god','world_tree'].flatMap(line =>
...
function runtimeFreshPet(form='adult') {
  ...
  try {
    ...
    Object.assign(state,{
      ...
      achievementsUnlocked:['age-10','age-25',...(FORMS[form]>=50 ? ['age-50'] : []),...(['dragon','phoenix','god','world_tree'].some(line=>form.startsWith(line)) ? ['rare-line-1'] : [])],
    });
    return state;
  } finally {
