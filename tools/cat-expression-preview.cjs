...
const FORMS = Object.freeze({adult:25,kitten:7,otemba:12,young:16,calm:40,elder:70,toddler:3,baby:1,dogAdult:25,puppy:7,wanpaku:12,youngDog:16,calmDog:40});
...
function runtimeFreshPet(form='adult') {
  ...
  try {
    ...
    Object.assign(state,{
      stage:'growing',speciesLine:['dogAdult','puppy','wanpaku','youngDog','calmDog'].includes(form)?'dog':'cat',stageIndex:runtime.api.stageForAge(FORMS[form]),ageTicks:FORMS[form]*20,
...
if (require.main === module) {
  ...
  if (!outputPath) {
    process.stderr.write('Usage: node tools/cat-expression-preview.cjs <output.html> [preset] [adult|kitten|otemba|young|calm|elder|toddler|baby|dogAdult|puppy|wanpaku|youngDog|calmDog]\n');
    process.exitCode=1;
  } else {
