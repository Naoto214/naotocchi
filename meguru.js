  ...
  root.installNaotocchiMeguru = function installNaotocchiMeguru(S) {
    ...
    const RENDER_TUNING = {
      ...
      occlusion: { min: 0.52, cover: 0.22, span: 0.55, inSpeed: 7, outSpeed: 3.4,
        // ランドマーク(たき・おおきな木…)を カメラの すぐ まえで ふさぐ ものも すける。
        // near より てまえ(dz)で、ランドマークの 絵の cover いじょうを かくす ときだけ。いちばん うすくて min
        landmark: { near: 650, cover: 0.2, span: 0.4, min: 0.28, maxDz: 2400, spotKey: 520, keyMin: 0.45 },
        // みちばたの ちいさな 絵文字(lane / field)は ふだん すけない。カメラの すぐ まえで
        // じぶんの 6わり より おおきく 見える ときだけ、じぶんを かくして いれば すける
        small: { layers: ['lane', 'field'], bigger: 0.6, min: 0.35, span: 0.12, box: [0.24, 0.8], cover: 0.25 } },
    ...
    // かたがわに はりだして えがく かたち(がけ・ビル)は、あたりの はこも その ぶん ずらす(side × px の なんばい)。
    // ずらさないと、えがいた がけが じぶんを かくして いても「かさなって いない」と はかって すけなかった
    const OCCLUDER_SHIFT = { cliff: 0.4, cliffwall: 0.4, seacliff: 0.4, building: 0.24, alleywall: 0.24, shopblock: 0.27 };
    ...
    function* buildWorldSteps(regionId, registry, opts = {}) {
      ...
      const DECO_LEAD_SIZE = { arch: 400, fern: 420 };
      for (const s of base.spots) {
    ...
    // 夜の もちあげ(地域ごと): light × mul、ただし floor より くらく しない。ジャングルは 林冠の したで 0.36 まで おちていた
    const NIGHT_LIFT = Object.freeze({ jungle: Object.freeze({ mul: 1.15, floor: 0.56 }) });
    // ジャングルの 夜: みきの あいだの つきあかりの もや と、ぬれた はの ひかり
    const LEAF_NIGHT = Object.freeze({ haze: '120,170,165', hazeA: 0.1, hazeTo: 0.55, glint: '200,240,225', glintA: 0.22, glints: 14 });
    ...
    // star_stop の 地平線の 霞(うえ 0.35 → 地平線 0.27 → した 0.95): 地平線で いちばん こく、地上の こい ところ でも 0.7
    const HORIZON_HAZE = Object.freeze([[0, 0], [0.27, 1], [0.58, 0.7], [1, 0]].map(Object.freeze));
    // ランドマークの レンズ まえ: 絵の たかさが がめんの from ばい を こえたら うすく、to ばい で えがかない。
    // ふつうに ながめる とき(その spot で むきあう)は 1.0〜1.6 ばい
    const LANDMARK_NEAR = Object.freeze({ from: 2.4, to: 3.2 });
    function landmarkNearAlpha(k) { return !(k > LANDMARK_NEAR.from) ? 1 : Math.max(0, 1 - (k - LANDMARK_NEAR.from) / (LANDMARK_NEAR.to - LANDMARK_NEAR.from)); }
    ...
    function createCanvasRenderer(o) {
      ...
      function drawDistantBelow(world, light, tl) {
        ...
        // 地上の いちばん こい ところ(0.4)まで 霞を あつく のこす(みはらしで うすい 灰緑の おびが 見えない ように)
        const hy0 = HOR - (y1 - HOR) * 0.35, hy1 = HOR + (y1 - HOR) * 0.95;
        const hg = ctx.createLinearGradient(0, hy0, 0, hy1); for (const [t, a] of HORIZON_HAZE) hg.addColorStop(t, hazeA(a));
      ...
      function drawCanopy(world) {
        ...
        if (k === 'leaves') { // うえと よこから おおきな はが せまる(みとおしが せまい)
          ...
          if (night) { // 夜: みきの あいだに つきあかりの もや(おくゆき)と、ぬれた はの ひかり(ばしょは きまり、ゆっくり またたく)
            const L = LEAF_NIGHT, y0 = HOR, y1 = HOR + (H - HOR) * L.hazeTo, g = ctx.createLinearGradient(0, y0, 0, y1);
            g.addColorStop(0, `rgba(${L.haze},0)`); g.addColorStop(0.35, `rgba(${L.haze},${L.hazeA})`); g.addColorStop(1, `rgba(${L.haze},0)`);
            ctx.fillStyle = g; ctx.fillRect(0, y0, W, y1 - y0);
            ctx.fillStyle = `rgba(${L.glint},${L.glintA})`; ctx.beginPath();
            for (let i = 0; i < L.glints; i++) { const gx = wrapN((i * 97) % W + pxN * 0.8, W), gy = HOR + (H - HOR) * (0.1 + ((i * 53) % 70) / 100), tw = 0.6 + 0.4 * Math.sin(curNow * 0.0012 + i * 1.7); const r = W * 0.012 * tw; ctx.moveTo(gx + r * 2, gy); ctx.ellipse(gx, gy, r * 2, r * 0.7, -0.4, 0, TAU); }
            ctx.fill();
          }
        }
      ...
      const FOOTPRINT = OCCLUDER_BOX, SWAY = SWAY_AMOUNT, OCCLUDES = OCCLUDER_LAYERS, SMALL_OCC = new Set(RENDER_TUNING.occlusion.small.layers);
      ...
      function draw(view, now) {
        ...
        const tl = TIME_LIGHT[e.time] || TIME_LIGHT.day; const wl = WEATHER_LIGHT[e.weather] || 0.9; let light = tl.light * wl * (mood.light || 1);
        // 夜に くらく なりすぎる 地域は すこし もちあげる(地形・みち・は が よめる さいていの あかるさ)
        { const nl = e.time === 'night' && NIGHT_LIFT[world.regionId]; if (nl) light = Math.max(light * nl.mul, nl.floor); }
        ...
        if (pp) {
          ...
          // 画面に 見えて いる ランドマークの はこ(ながめる ものを てまえの 木で ふさがない)
          const LMO = RENDER_TUNING.occlusion.landmark, lmBox = [];
          for (const it of items) {
            // ながめる もの: ランドマーク と、じぶんの ちかくの spot の deco の 主役(つりばし など)
            if (it.kind !== 'prop' || it.p.dz > LMO.maxDz) continue;
            const spotKey = !it.o.landmark && it.o.deco && it.o.keep && Math.hypot(it.o.x - player.x, it.o.z - player.z) < LMO.spotKey;
            if (!it.o.landmark && !spotKey) continue;
            const px2 = it.o.size * it.p.s, fp = it.o.landmark ? FOOTPRINT.landmark : FOOTPRINT[it.o.struct || 'glyph'] || FOOTPRINT.glyph;
            if (landmarkNearAlpha(px2 / H) < 1) continue; // レンズの なかの ランドマークは ながめる もの では ない
            const l = Math.max(0, it.p.sx - px2 * fp[0]), r = Math.min(W, it.p.sx + px2 * fp[0]), t = Math.max(0, it.p.sy - px2 * fp[1]), b = Math.min(H, it.p.sy);
            if (r - l > 8 && b - t > 8) lmBox.push({ o: it.o, dz: it.p.dz, near: spotKey, l, r, t, b, area: (r - l) * (b - t) });
          }
          for (const it of items) {
            if (it.kind !== 'prop') continue;
            const smallNear = !OCCLUDES.has(it.o.layer) && it.o.emoji && SMALL_OCC.has(it.o.layer) && it.o.size * it.p.s > ppx * RENDER_TUNING.occlusion.small.bigger;
            if ((!OCCLUDES.has(it.o.layer) && !smallNear) || (it.o.layer === 'struct' && it.o.size <= 140)) continue;
            const fade = fadeOf(it.o);
            let want = 1;
            // 木 の あいだ から は 見える が、岩 や がけ(地形)の むこう は 見えない ので すけない
            if (lmBox.length && !it.o.landmark && !it.o.keep && it.p.dz < LMO.near && STRUCT_ROLE[it.o.struct] !== 'terrain') {
              const px2 = it.o.size * it.p.s;
              const fp = FOOTPRINT[it.o.struct || 'glyph'] || FOOTPRINT.glyph;
              const cx = it.p.sx + (it.o.side < 0 ? -1 : 1) * (OCCLUDER_SHIFT[it.o.struct] || 0) * px2;
              const l = cx - px2 * fp[0], r = cx + px2 * fp[0], t = it.p.sy - px2 * fp[1], b = it.p.sy;
              for (const m of lmBox) {
                if (m.o === it.o || it.p.dz > (m.near ? m.dz - 40 : m.dz * 0.7)) continue;
                const ox = Math.min(r, m.r) - Math.max(l, m.l), oy = Math.min(b, m.b) - Math.max(t, m.t);
                if (ox <= 0 || oy <= 0) continue;
                const cover = ox * oy / m.area;
                if (cover > LMO.cover) want = Math.min(want, lerp(1, m.near ? LMO.keyMin : LMO.min, Math.min(1, (cover - LMO.cover) / LMO.span)));
              }
            }
            if (it.p.dz < pp.dz) {
              const px2 = it.o.size * it.p.s;
              // ちいさな 絵文字は 絵の まんなか だけ を あたりに する(はしが かすめた だけ では すけない)
              const fp = smallNear ? RENDER_TUNING.occlusion.small.box : FOOTPRINT[it.o.struct || (it.o.landmark ? 'landmark' : 'glyph')] || FOOTPRINT.glyph;
              const cx = it.p.sx + (it.o.side < 0 ? -1 : 1) * (OCCLUDER_SHIFT[it.o.struct] || 0) * px2;
              const l = cx - px2 * fp[0], r = cx + px2 * fp[0];
              ...
              if (ox > 0 && oy > 0) {
                ...
                const c0 = smallNear ? RENDER_TUNING.occlusion.small.cover : RENDER_TUNING.occlusion.cover;
                const oc = smallNear ? RENDER_TUNING.occlusion.small : RENDER_TUNING.occlusion;
                if (cover > c0) want = Math.min(want, lerp(1, smallNear ? oc.min : OCC_MIN, Math.min(1, (cover - c0) / oc.span)));
              }
            }
        ...
        for (const it of drawList) {
          if (it.kind === 'prop') {
            ...
            if (it.o.landmark) {
              // レンズの なかに はいるほど ちかい ランドマーク(その うえに たっている)は、がめんを ふさぐ 板に なる ので うすく → えがかない
              const lmA = occ * landmarkNearAlpha(it.o.size * it.p.s / H);
              if (lmA <= 0.02) { curFog = 0; continue; }
              // ランドマークも おなじ: すける ものは べつの 板へ
              if (fadeInk && lmA < 0.9) { const main = ctx; ctx = fadeInk; drawLandmark(it.o.landmark, it.p, it.o.size, light, world); ctx = main; fadeMark(it, lmA, 0.9, 1.6); curFog = 0; continue; }
              ctx.globalAlpha = lmA; drawLandmark(it.o.landmark, it.p, it.o.size, light, world); ctx.globalAlpha = 1; curFog = 0; continue;
            }
            if (it.o.struct) {
              let a = occ * nearFade;
